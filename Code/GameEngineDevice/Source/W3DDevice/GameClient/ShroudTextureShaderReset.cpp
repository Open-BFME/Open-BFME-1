// cl: /DNDEBUG /MD /EHsc
// Retail 0x007C4660, 625 B. W3DShaderManager.cpp (same directory) documents
// this exact source under "ShroudTextureShader::reset" (present-unmatched)
// with the BFME_SET_RS/BFME_SET_TSS macro shapes this body's disassembly
// matches field-for-field: the D3DRS_ZFUNC/D3DCMP_LESSEQUAL global slot,
// then the runtime-stage D3DTSS_TEXCOORDINDEX/D3DTSS_TEXTURETRANSFORMFLAGS
// pair. Unlike the sibling FlatShroudTextureShader::reset
// (FlatShroudTextureShaderReset.cpp, landed), this body's BoxSetTexture
// call is UNCONDITIONAL -- no `m_stageOfSet<MAX_TEXTURE_STAGES` guard around
// the StageTextureRef scope, matching W3DShaderManager.cpp's narrative
// ShroudTextureShader::reset verbatim (no guard there either).
// This TU is a self-contained shim (own StringClass/Device/DX8Wrapper
// stand-ins), same style as FlatShroudTextureShaderReset.cpp, so it does not
// depend on the still-unmatched narrative file compiling.
class StringClass {
	char *m_Buffer;
...
// ?reset@ShroudTextureShader@@EAEXXZ
