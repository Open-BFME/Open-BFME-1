// cl: /DNDEBUG /MD /EHsc
// twin of Gen006C6E20Terrain::query (retail 0x006C6E20): convert two terrain
// coordinates and query the map plane, different member-thunk callee.

typedef bool Bool;

extern void j_00007851(void);

#define BFME_AT(type, address) (*reinterpret_cast<type *>(address))

class Gen006C6EC0Map
{
public:
...
private:
	...
	Gen006C6EC0Map *m_map;
};

Bool Gen006C6EC0Terrain::query(float first, float second)
{
	if (m_map == 0)
		return false;
	...
	typedef Bool (Gen006C6EC0Map::*MemberThunk)(int, int) const;
	union
	{
		void (*function)(void);
		MemberThunk member;
	} thunk;
	thunk.function = j_00007851;
