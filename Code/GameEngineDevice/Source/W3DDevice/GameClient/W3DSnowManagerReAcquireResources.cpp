...
//
// Retail 0x00723FB0 -- W3DSnowManager::ReAcquireResources.  Identified against
// reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/GameEngineDevice/Source/
// W3DDevice/GameClient/W3DSnow.cpp:77, whose body this reproduces step for
// step: ReleaseResources, the TheWeatherSetting->m_snowEnabled early-out, the
// point-sprite CreateVertexBuffer path, the DX8IndexBufferClass fallback with
// the same 0/3/2 + 2/0/1 quad index fill over SNOW_BATCH_SIZE, then the snow
// texture fetch and the m_dwBase/m_dwDiscard/m_dwFlush trio.  The member order
// below is upstream's declaration order (W3DSnow.h:48-53).
//
// The 16-byte caller at 0x00725600 first invokes the matched SnowManager::init
// body and then tail-jumps here; that tail-pair forwarder still calls the
// address-derived pin in reverse/symbols.csv, which is a different symbol from
// the body defined here.
//
// ReAcquireResources is not virtual upstream, and the shim keeps it that way:
// declaring one virtual here would emit a ??_7W3DSnowManager@@6B@ COMDAT whose
// single slot disagrees with the one W3DSnowManagerUpdate.cpp emits.
...
#pragma comment(linker, "/alternatename:?ReleaseResources@W3DSnowManager@@QAEXXZ=?j_000491c5@@YAXXZ")
