...
// readable body of ?setCameraTransform@W3DView@@: Code/GameEngineDevice/Source/W3DDevice/GameClient/W3DView.cpp
...
void W3DView::setCameraTransform()
{
	...
	if (m_cameraConstraintValid && m_applyCameraConstraints)
	{
		...
		pos.y = getPosition()->y;
		pos.z = getPosition()->z;
		pos.x = clampf( pos.x, m_cameraConstraint.lo.x, m_cameraConstraint.hi.x );
		pos.y = clampf( pos.y, m_cameraConstraint.lo.y, m_cameraConstraint.hi.y );
		setPosition( &pos );
	}

	if ( TheWritableGlobalData->m_debugCamera )
		m_3DCamera->Set_View_Plane( TheWritableGlobalData->m_debugCameraFOV, -1.0f );
	else
		m_3DCamera->Set_View_Plane( m_FOV, -1.0f );
