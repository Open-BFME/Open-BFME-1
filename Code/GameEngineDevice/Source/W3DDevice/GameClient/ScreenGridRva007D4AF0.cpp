// cl: /DNDEBUG /MD
#include <math.h>
// Retail 0x007D4AF0: dynamic index/vertex grid update; original method name unknown.
...
struct GridGlobals{char pad[0x54];float scale;};GridGlobals*GridScaleRva007D4AF0();
extern float GridFrameScale;static float GridPhase0,GridPhase1;
...
class ScreenGridRva007D4AF0{char pad[0x20];float scrollX,scrollY,spread;public:void update(DynamicIBAccessClass*,int,BoxDynamicVBAccessClass*,int,int,int,float,float,float,float,Coord2D*);};
void ScreenGridRva007D4AF0::update(DynamicIBAccessClass*ib,int a,BoxDynamicVBAccessClass*vb,int b,int columns,int rows,float left,float top,float width,float height,Coord2D*dims){
 ++columns;++rows;float scale=GridFrameScale*0.0125f;
 if(scale>6.0f)scale=6.0f;else if(scale<0.6f)scale=0.6f;
 float amplitude=scale*GridScaleRva007D4AF0()->scale;
 {
 ...
 GridPhase0+=0.05f;GridPhase1+=0.05f;
 {
  ...
  for(int y=0;y<rows;++y){
   ...
   for(int x=0;x<columns;++x){
    ...
    p->v3=0;
    p->u3=(0.5f-0.5f*(float)sin((fx+fy)*GridWave0Frequency+GridPhase0*GridWave0Phase))*GridWave0Amplitude;
    float wave=(0.5f-0.5f*(float)sin((fx+fy)*GridWave1Frequency+GridPhase1*GridWave1Phase))*GridWave1Amplitude;
