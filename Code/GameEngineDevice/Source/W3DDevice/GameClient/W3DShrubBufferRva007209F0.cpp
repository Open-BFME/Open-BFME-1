...
// W3DShrubBuffer method at retail 0x007209F0, twin of W3DTreeBuffer's at 0x00736940: finds or adds the shrub
// type for a template's tree draw module data under the template's own name; returns its index, or -2.
...
struct Rva007209F0TreeDrawData
{
...
public:
	...
	virtual const Rva007209F0TreeDrawData *getAsW3DTreeDrawModuleData() const;
};
...
public:
	...
	Int rva007209F0(const AsciiString &templateName, Int shadowKind, const AsciiString &textureName);

private:
	unsigned char prefix[0x1e1cd1];
	bool m_needUpdate;
	unsigned char alignment[2];
	Rva007209F0TreeType m_treeTypes[64];
	Int m_numTreeTypes;
};

// ?rva007209F0@W3DShrubBuffer@@QAEHABVAsciiString@@H0@Z
Int W3DShrubBuffer::rva007209F0(const AsciiString &templateName, Int shadowKind, const AsciiString &textureName)
{
	...
	if (module == 0)
		return -2;
	const Rva007209F0TreeDrawData *data = module->getAsW3DTreeDrawModuleData();
	if (data == 0) {
		if (_bfme_debugReportingEnabled()) {
			_bfme_debugRecordCallsite(1);
			TheBfmeAwakenDebug->v60();
			TheBfmeAwakenDebug->v6c(0, 0)->v38("Shrub ")->v38(tmpl->m_name.str())
	...
	if (index < 0) {
		index = addTreeType(data->m_modelName, data->m_nameC, data, shadowKind, textureName, tmpl->m_name);
		if (index >= 0)
			m_needUpdate = true;
	}
	return index;
