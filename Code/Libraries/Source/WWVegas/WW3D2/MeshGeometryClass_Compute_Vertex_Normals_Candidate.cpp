...
static __forceinline void Copy_Skin_Normals(MeshGeometryClass *mesh, Vector3 *vnorm)
{
	float *skin_normals = mesh->Slot48->Get_Array();
	int vertex_count = *reinterpret_cast<int *>(reinterpret_cast<char *>(mesh) + 0x28);
	int vidx = 0;
	int z_offset = vertex_count + vertex_count;
	if (vertex_count > 0) {
		do {
			skin_normals[vidx] = vnorm[vidx].X;
			skin_normals[mesh->VertexCount + vidx] = vnorm[vidx].Y;
			skin_normals[z_offset + vidx] = vnorm[vidx].Z;
			vidx++;
		} while (vidx < mesh->VertexCount);
	}
}

static __forceinline float *JY_Address(Vector3 *vnorm, const uint32 *shadeIx,
	TriIndex *poly, int pidx)
{
	return &vnorm[shadeIx[poly[pidx].J]].Y;
}

static inline void Accumulate(float *dst, float value)
{
	*dst += value;
}
