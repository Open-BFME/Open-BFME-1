...
// changed. Blend starts the X dot product with Z; Combo starts it with X.
// The template selects that order at compile time. These are reconstruction
// constraints, not original volatile fields.
template <bool FirstXProduct>
static WWINLINE void HTree_Post_Multiply(Matrix3D &a, const Matrix3D &b)
{
	...
	if (FirstXProduct) {
		tmpX = (float)((a[0].X * (*(const volatile float *)&b[0].X) + a[0].Z * b[2].X) + a[0].Y * b[1].X);
	} else {
	...
	for (int piv_idx=1; piv_idx < NumPivots; piv_idx++) {
		...
		if (piv_idx < num_anim_pivots) {
			...
			if (got0 || got1) {
				...
				Matrix3D mtx;
				::Build_Matrix3D(q,mtx);
				HTree_Post_Multiply<false>(pivot->Transform,mtx);
			}
...
//				WWASSERT(WWMath::Fabs( weight_total - 1.0 ) < WWMATH_EPSILON);
				...
				HTree_Post_Multiply<true>(pivot->Transform,Build_Matrix3D(q0,mtx));
			}

			pivot->IsVisible = false;

			// BFME sums the visited animations' fades alongside visibility and
			// averages them when the loop ends, including an early visible exit.
