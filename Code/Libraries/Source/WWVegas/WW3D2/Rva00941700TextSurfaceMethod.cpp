...
// Direct calls from 0x00941C60 establish this helper's owner ABI and four arguments.
// cl: /DNDEBUG /MD /EHsc
...
public:
	...
	__forceinline int Get_Char_Metric_Sum(unsigned short character)
	{
		const FontCharsClassCharDataStruct *data = Get_Char_Data(character);
		if (data != 0)
	...
	unsigned short *glyphs, bool justCalcExtents)
{
	...
	for (int index = 0; text[index] != 0; index++) {
		text_width += font->Get_Char_Metric_Sum(glyphs[index]);
	}
