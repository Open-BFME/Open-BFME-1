...
{
public:
	BfmeMsgVJR(char *buf, int n) throw();
	void done() throw();
	void setError(int code) throw();
	char hasError() throw();
	int getError() throw();
...
public:
	...
	union
	{
		BfmeSinkErrorVJQ *m_bfme1cError;
		BfmeSinkZero18VJR *m_bfme1cZero;
	};
	char m_pad20[8];
	int m_bfme28;
	int m_bfme2c;
...
{
	BfmeMsgStorageVJR storage;
	BfmeMsgVJH *msg = (BfmeMsgVJH *)&storage;
	msg->BfmeMsgVJH::BfmeMsgVJH(m_bfmeBuf, 0x400);
	if (m_bfme34 && m_bfme30 >= 3)
	{
		m_bfme10->sendFour(msg, a1, a2, a3, a4);
		m_bfme14->send(msg, BfmeGameBrowserVJRCallback, this, m_bfme6dc);
		msg->BfmeMsgVJH::~BfmeMsgVJH();
		return;
	}
	if (m_bfme35)
	{
		...
		{
			BfmeSinkZero18VJR *sink = m_bfme1cZero;
			sink->sendZero(0);
			msg->BfmeMsgVJH::~BfmeMsgVJH();
			return;
		}
