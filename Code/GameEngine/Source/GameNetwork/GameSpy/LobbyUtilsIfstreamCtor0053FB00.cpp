// Open-BFME7: the (const char*, int) basic_ifstream<char> constructor that
// LobbyUtils.cpp instantiated at 0x0053FB00 (201 B): the same shell as the
// stlport default constructor (0x0084C240) with the filebuf constructed from
// the name and mode|in through the ILT at 0x0004A958 (0x0093C660).
...
public:
  basic_filebuf(const char *name, int mode);
...
public:
  basic_ifstream(const char *name, int mode)
      : basic_ios<CharT, Traits>(), basic_istream<CharT, Traits>(0), buf_(name, mode | 8) {
    this->init(&buf_);
  }
