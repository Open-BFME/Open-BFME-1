...
// stlport
//
// Retail 0x0064ECC0: PeerThread's joinRoomCallback, complete 626-byte body.
// Identity: retail pushes 0x0064ECC0 as the callback argument, with the
// PeerThreadClass pointer as param and PEERTrue blocking, at
// peerJoinGroupRoom call sites 0x0064F778 and 0x00650BD3, and at the
// 0x00858B70 join call at 0x006503B8 (ZH PeerThread.cpp passes
// joinRoomCallback to peerJoinGroupRoom and peerJoinStagingRoom).  This TU keeps the measured callback
...
// 0x00643BC0 and 0x00643C10 are the ledger's byte-matched dump bodies
// ?d_00643bc0 and ?d_00643c10 (DualIndexedDispatchThunk.cpp): thiscall over
// the PeerThreadClass pointer with one PEER argument, callee-popped.  They are
// called through their dump symbols, typed as fastcall with the PEER wrapped
// in a one-word struct so it stays on the stack and ECX carries the owner.
extern void d_00643bc0();
extern void d_00643c10();

struct DualIndexedPeerArg
{
	PEER peer;
};

typedef void (__fastcall *DualIndexedDispatchCall)(void *owner,
	DualIndexedPeerArg arg);
	...
	RoomType roomType, void *param)
{
	...
	if (success)
	{
		DualIndexedPeerArg arg = { peer };
		reinterpret_cast<DualIndexedDispatchCall>(d_00643bc0)(t, arg);
		reinterpret_cast<DualIndexedDispatchCall>(d_00643c10)(t, arg);
