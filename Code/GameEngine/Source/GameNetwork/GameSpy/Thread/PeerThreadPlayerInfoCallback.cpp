...
// stlport
//
// BFME's PeerResponse carries the 0x330-byte player payload.  Keep this
// callback's ABI slice local: the public Zero Hour header has the same
// callback contract but a smaller response layout.
...
void playerInfoCallback(PEER peer, RoomType roomType, const char *nick,
	UnsignedInt IP, Int profileID, void *param)
{
	if (!nick)
		return;
	PeerResponse resp;
