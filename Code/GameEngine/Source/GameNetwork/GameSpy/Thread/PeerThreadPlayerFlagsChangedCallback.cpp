...
// stlport
//
// Keep the callback's BFME PeerResponse ABI local.  The retail callback
// receives the old and new flags but obtains the current player record before
// queuing the response, so those two inputs do not otherwise participate.
...
void playerFlagsChangedCallback(PEER peer, RoomType roomType,
	const char *nick, Int oldFlags, Int newFlags, void *param)
{
	if (!nick)
		return;
	PeerResponse resp;
	resp.peerResponseType = 14;
	resp.nick = nick;
