...
// stlport
//
// TU-local reconstruction of the BFME room-key callback and its 0x330-byte
// PeerResponse ABI.  The peer RoomType values are TitleRoom=0, GroupRoom=1,
// and StagingRoom=2.
...
{
public:
	void trackStatsForPlayer(RoomType, const char *, const char *, const char *);
...
void roomKeyChangedCallback(PEER peer, RoomType roomType, const char *nick,
	const char *key, const char *val, void *param)
{
	...
	getPlayerInfo(t, peer, nick, resp.player.profileID, resp.player.IP,
		...
		resp.player.roomType, resp.player.flags, resp.player.rank1v1,
