...
// CostModifierUpgrade::upgradeRemovalImplementation (0x002D4770, same
// file family, CostModifierUpgradeRemoval.cpp). Same Player/ModuleData
...
// ObjectModule-inherited getObject()/getModuleData() accessors from this
// override compiles to the this-0x8/this-0xc offsets seen in the body),
// but this slot increments the player's counter instead of decrementing
// it and does not gate on isAlreadyUpgraded() or call setUpgradeExecuted.
...
void CostModifierUpgrade::upgradeImplementation()
{
	...
	{
		player->m_methods.m_counter.inc();
	...
	{
		player->m_methods.m_entries.bfmeAdd(
