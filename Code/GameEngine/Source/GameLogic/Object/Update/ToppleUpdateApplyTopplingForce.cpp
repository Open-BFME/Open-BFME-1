// Retail RVA 0x002B1BC0, 832 bytes.  The shim name is the pinned body identity;
// the forwarding thunk lives in Common/RTS/ToppleUpdateApplyTopplingForceThunk.cpp.
// cl: /DNDEBUG /MD /EHsc
// stlport
...
extern void j_0000666d();

class ModelConditionFlags
{
public:
	typedef bool (ModelConditionFlags::*TestMethod)(int) const;

	bool test(int bit) const
	{
		union { void (*raw)(void); TestMethod member; } fn;
		fn.raw = j_0000666d;
		return (this->*fn.member)(bit);
	}
};
...
typedef Object *(ThingFactory::*ThingFactoryNewObjectCall)(
	const ThingTemplate *, Team *, const ObjectStatusBits *, int);
...
void ToppleUpdateApplyTopplingForceShim::apply(const Coord3D *toppleDirection, Real toppleSpeed, UnsignedInt options)
{
	...
	{
		ThingTemplate *ttn = TheThingFactory->findTemplate(d->m_stumpName);
		ObjectStatusBits bits;
		union { void (*raw)(void); ThingFactoryNewObjectCall member; } newObjectCall;
		newObjectCall.raw = j_0004494a;
		Object *stump = (TheThingFactory->*newObjectCall.member)(
			ttn, 0, &bits, 0);
		if (stump)
		{
