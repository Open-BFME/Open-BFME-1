...
// stlport
// Open-BFME: AICommandInterface path-command, retail 0x00153240, 231 bytes.
//
// >=93% difflib twin of aiBfmeCommand9FollowPath
// (AICommandInterface_aiBfmeCommand9FollowPath.cpp, retail 0x00153120,
// 220 bytes): identical build-parms/copy-path/set-object/aiDoCommand shape,
// but this body takes one extra pointer argument (ret 0x10 here vs 0xc on
// the twin) and writes two object fields (m_obj and m_otherObj) instead of
// one, and the AICMD constant differs (0x24 here, not 9). Address-derived
// name pending the real command enumerator/caller.
...
enum AICommandType { AICMD_BFME_36 = 0x24 };
