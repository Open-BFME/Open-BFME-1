...
// where the twin calls oneArgument(play) then twoArguments(0, 0) as two separate vtable
// slots (0x18c then 0x188), this body makes a single vtable-0x188 call passing the
// float's raw bits through the int parameter: twoArguments(1, play-bits). Real name
// unproven -- landed under an address-derived name; identity evidence: identical
// TheScriptEngine/TheTacticalView globals (0x012F076C, 0x012F1600) and identical
...
{
protected:
	void doCameraTether(const AsciiString &unit, Bool snapToUnit, float play);
};

// address-derived name (real ScriptActions method unproven)
void Rva002EFA10Owner::doCameraTether(const AsciiString &unit, Bool snapToUnit, float play)
{
