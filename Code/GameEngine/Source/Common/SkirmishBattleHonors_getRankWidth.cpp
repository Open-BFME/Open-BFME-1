...
public:
	int m_pad[3];
	int m_rankPoints[11];
	int getRankWidth( AsciiString name, int rank ) const;
};

int SkirmishBattleHonors::getRankWidth( AsciiString name, int rank ) const
{
	if ( rank == 10 )
		return 0;
	return m_rankPoints[rank + 1] - m_rankPoints[rank];
}
