...
{
    return m_data ? m_data->data : reinterpret_cast<const unsigned short *>(L"");
}
...
Xfer &Xfer::operator==(UnicodeString &us)
{
    ...
    if (receiver->isStoring())
    {
        ...
        if (length >= 255)
        {
            ...
            receiver->transfer(reinterpret_cast<void *>(0x75737472), &marker, 1);
    ...
    else
    {
        ...
        if (length != 0)
        {
            receiver->transfer(0, reinterpret_cast<StringBase<unsigned short> *>(&us)->getBufferForRead(length), length * 2);
            reinterpret_cast<StringBase<unsigned short> *>(&us)->getBufferForRead(length)[length] = 0;
        }
        else
            reinterpret_cast<StringBase<unsigned short> *>(&us)->clear();
    }
    return *this;
}
