...
// to the global setting at GlobalData+0xB14, so that path returns early rather
// than sharing an epilogue. A present value is floored at zero. The key lives in
// its own scope so it dies after the lookup and before the end() comparison,
// which is the order retail uses.
//
...
{
public:
	unsigned char m_unreconstructed_00[0xB14];
