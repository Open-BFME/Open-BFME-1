// cl: /DNDEBUG /MD
// Retail 0x0048E480, 212 bytes. Address-derived owner: hover/fade helper
// using TheGameClient(+0x68), TheMouse+0x4D10/0x4D14, TheGameLogic+0x3C,
// TheInGameUI(+0x18C). Identity not recovered from an image name.

class GameClient
{
public:
	virtual void slot00(); virtual void slot01(); virtual void slot02();
	virtual void slot03(); virtual void slot04(); virtual void slot05();
	virtual void slot06(); virtual void slot07(); virtual void slot08();
	virtual void slot09(); virtual void slot10(); virtual void slot11();
	virtual void slot12(); virtual void slot13(); virtual void slot14();
	virtual void slot15(); virtual void slot16(); virtual void slot17();
	virtual void slot18(); virtual void slot19(); virtual void slot20();
	virtual void slot21(); virtual void slot22(); virtual void slot23();
	virtual void slot24(); virtual void slot25();
	virtual unsigned int getFrame();
};

class InGameUI
{
public:
	virtual void slot00(); virtual void slot01(); virtual void slot02();
	virtual void slot03(); virtual void slot04(); virtual void slot05();
	virtual void slot06(); virtual void slot07(); virtual void slot08();
	virtual void slot09(); virtual void slot10(); virtual void slot11();
	virtual void slot12(); virtual void slot13(); virtual void slot14();
	virtual void slot15(); virtual void slot16(); virtual void slot17();
	virtual void slot18(); virtual void slot19(); virtual void slot20();
	virtual void slot21(); virtual void slot22(); virtual void slot23();
	virtual void slot24(); virtual void slot25(); virtual void slot26();
	virtual void slot27(); virtual void slot28(); virtual void slot29();
	virtual void slot30(); virtual void slot31(); virtual void slot32();
	virtual void slot33(); virtual void slot34(); virtual void slot35();
	virtual void slot36(); virtual void slot37(); virtual void slot38();
	virtual void slot39(); virtual void slot40(); virtual void slot41();
	virtual void slot42(); virtual void slot43(); virtual void slot44();
	virtual void slot45(); virtual void slot46(); virtual void slot47();
	virtual void slot48(); virtual void slot49(); virtual void slot50();
	virtual void slot51(); virtual void slot52(); virtual void slot53();
	virtual void slot54(); virtual void slot55(); virtual void slot56();
	virtual void slot57(); virtual void slot58(); virtual void slot59();
	virtual void slot60(); virtual void slot61(); virtual void slot62();
	virtual void slot63(); virtual void slot64(); virtual void slot65();
	virtual void slot66(); virtual void slot67(); virtual void slot68();
	virtual void slot69(); virtual void slot70(); virtual void slot71();
	virtual void slot72(); virtual void slot73(); virtual void slot74();
	virtual void slot75(); virtual void slot76(); virtual void slot77();
	virtual void slot78(); virtual void slot79(); virtual void slot80();
	virtual void slot81(); virtual void slot82(); virtual void slot83();
	virtual void slot84(); virtual void slot85(); virtual void slot86();
	virtual void slot87(); virtual void slot88(); virtual void slot89();
	virtual void slot90(); virtual void slot91(); virtual void slot92();
	virtual void slot93(); virtual void slot94(); virtual void slot95();
	virtual void slot96(); virtual void slot97(); virtual void slot98();
	virtual void disableTooltipsUntil(unsigned int frame);
};

struct MouseState
{
	char m_pad[0x4D10];
	int x;
	int y;
};

struct GameLogicState
{
	char m_pad[0x3C];
	int frame;
};

struct RegionBox
{
	char m_pad00[8];
	int x0;
	int y0;
	int x1;
	int y1;
	unsigned int color;
};

extern GameClient *TheGameClient;
extern MouseState *TheMouse;
extern GameLogicState *TheGameLogic;
extern InGameUI *TheInGameUI;

class Rva0048E480
{
public:
	virtual void slot00();
	virtual void slot01();
	virtual void slot02();
	virtual void slot03();
	virtual void earlyOut();
	void update();

	char m_pad04[4];
	RegionBox *m_box;
	char m_pad0C[0x0C];
	unsigned int m_enabled;
	unsigned int m_guard;
	unsigned int m_stamp;
};

void Rva0048E480::update()
{
	if (!m_enabled)
		return;

	unsigned int frame = TheGameClient->getFrame();
	unsigned int guard = m_guard;
	if (guard && guard < frame)
	{
		earlyOut();
		return;
	}

	int mx = TheMouse->x;
	int my = TheMouse->y;
	RegionBox *box = m_box;
	if (mx > box->x0 && mx < box->x1 && my > box->y0 && my < box->y1
		&& frame > m_stamp)
		m_stamp = frame + 1;

	unsigned int stamp = m_stamp;
	if (frame > stamp)
	{
		int delta = (int)(frame - stamp);
		unsigned int alpha;
		if (delta > 30)
			alpha = 0;
		else
			alpha = (unsigned int)(255 - (delta * 255) / 30);
		if (!guard && alpha < 0x40u)
			alpha = 0x40;
		box->color = alpha << 24;
	}
	else
		box->color = 0xFF000000;

	TheInGameUI->disableTooltipsUntil(TheGameLogic->frame + 2);
}
