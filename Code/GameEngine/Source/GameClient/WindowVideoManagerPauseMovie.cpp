...
// WindowVideoManager::pauseMovie, retail 0x00498730, 103 bytes.
//
// Sibling of WindowVideoManagerResumeMovie.cpp / StopMovie.cpp -- same
// find()-based single lookup (hideMovie's shape) and the same inlined
// setWindowState() shape, called here with WINDOW_VIDEO_STATE_PAUSE.
...
void WindowVideoManager::pauseMovie( GameWindow *win )
{
	...
	if(it != m_playingVideos.end())
	{
		...
		if(winVid)
			((BfmePauseMovie::WindowVideo *)winVid)->setWindowState(WINDOW_VIDEO_STATE_PAUSE);
	}
}
