...
// WindowVideoManager::stopMovie, retail 0x004988B0, 137 bytes.
//
// Sibling of WindowVideoManagerResumeMovie.cpp -- same find()-based single
// lookup (hideMovie's shape) and the same inlined setWindowState() shape,
// called here with WINDOW_VIDEO_STATE_STOP.
...
void WindowVideoManager::stopMovie( GameWindow *win )
{
	...
	if(it != m_playingVideos.end())
	{
		...
		if(winVid)
			((BfmeStopMovie::WindowVideo *)winVid)->setWindowState(WINDOW_VIDEO_STATE_STOP);
	}
}
