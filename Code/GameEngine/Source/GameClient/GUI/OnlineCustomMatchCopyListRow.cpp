...
//
// Retail 0x00538290, 277 bytes. Stdcall pack of a listing record: strncpy a
// 255-char name into a zeroed 256-byte buffer, then StringBase<char>::set the
// source name, the truncated copy, and the string at +0x34, plus the dword
// fields at +0xF4/+0xF8/+0xFC/+0x104/+0x10C/+0x110/+0x114/+0x324/+0x328/+0x32C.
...
struct CustomMatchListRow
{
	AsciiString name;
	AsciiString truncatedName;
	AsciiString extra;
	int field0C;
	int field10;
	int field14;
...
void __stdcall copyCustomMatchListRow( const CustomMatchListSource *src, CustomMatchListRow *dst )
{
	char buf[ 256 ] = { 0 };
	...
	AsciiString *extraSlot = &dst->extra;
