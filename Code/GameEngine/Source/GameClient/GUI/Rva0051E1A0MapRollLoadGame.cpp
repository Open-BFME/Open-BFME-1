...
public:
	...
	virtual void slot56();
	virtual Bool slot57(AsciiString name, int a, int b, int c);
	virtual void slot58();
	virtual void slot59();
	virtual Bool slot60();
	virtual void slot61();
...
public:
	...
	unsigned char m_rest[0xBB8 - 0xA7E];
	Bool m_unidentifiedBB8;
};
extern GlobalData *TheWritableGlobalData;

// The body at 0x0051E1A0 is the BFME load-game Map_Roll step.  Its
// address-derived name is retained because the existing 0x003BE150 twin is
// already the distinct ZH-shaped mapRollLoadGame body.
int rva0051E1A0MapRollLoadGame(int, bool start)
{
	const bool go = start;
	int result = 1;
	if (go)
	{
		TheWritableGlobalData->m_unidentifiedBB8 = false;
		if (!TheDisplay->slot57(AsciiString("Map_Roll"), 0x2000C0, -1, -1))
			result = 3;
		TheDisplay->rva002ED2E0(0.0f, 0.0f, 1.0f, 1.0f);
	}
	else
	{
		if (!TheDisplay->slot60())
			goto setResult3;
		if (TheDisplay->slot62())
			goto setResult3;
		if (!TheWritableGlobalData->m_unidentifiedA7D)
			goto done;
	setResult3:
		TheWritableGlobalData->m_unidentifiedBB8 = false;
		result = 3;
