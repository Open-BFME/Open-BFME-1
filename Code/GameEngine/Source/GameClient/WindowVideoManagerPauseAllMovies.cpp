...
// WindowVideoManager::pauseAllMovies, retail 0x00499440, 188 bytes.
//
// Sibling of WindowVideoManagerStopAllMovies.cpp -- same BFME WindowVideo
// layout (no video-stream member, m_state at +0x10) and the same inlined
// setWindowState() shape, called here with WINDOW_VIDEO_STATE_PAUSE so only
// the PLAY/PAUSE branch is live; the flag stores at the end are swapped
// (m_pauseAllMovies = TRUE then m_stopAllMovies = FALSE) to match the source.
...
void WindowVideoManager::pauseAllMovies( void )
{
	...
	while(it != m_playingVideos.end())
	{
		...
		if(winVid)
			((BfmePauseAllMovies::WindowVideo *)winVid)->setWindowState(WINDOW_VIDEO_STATE_PAUSE);
		it++;
	}
