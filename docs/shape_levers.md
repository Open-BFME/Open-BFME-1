...
|---|---|
| Retail zero-extends a narrow field with `xor edx,edx` then `mov dx,word ptr [m]` (or `mov dl`) and later stores or tests the full register, while ours emits one `movzx` and is two bytes short | Look for a narrow LOCAL that lives in memory. At `0x000CFF80` four sessions widened the field (casts, `/G5`-`/G7`, `/O1`, bit-fields) and all got `movzx`; the exact spelling was `unsigned short key = input->key; list.erase(&key);` -- a short local whose address is taken, which VC7.1 loads narrow and stores as a dword into the dead parameter slot. `bfmeInsertMask` at `0x00373E30` shows the same shape for a short loop variable kept both in `di` and on the stack. It does NOT reproduce a byte pushed straight to a call (`0x0033C9E0`): every argument type, cast and `/G`/`/O` flag tried there still gave `movzx`. |
| Every byte matches except that a hard-coded global address and the displacements reading through it are all shifted by a constant | Stop writing the address as a numeric literal. A literal is a plain immediate the byte gate has to reproduce exactly, and MSVC 7.1 is free to re-base it: it normalises a cursor to the LOWEST offset the loop reads, so `p = (T *)0x01303808 + 1` with `p[-1]` comes back as base `0x01303808` with `[edi]`, and no spelling of the initialiser (`+1`, `&buf[1]`, a struct field, a volatile pointee, a second pointer for the negative access, inline asm) moves it. Declare the global as an `extern` array instead: both references become DIR32 sites the patcher fills from retail, so the absolute values stop mattering and only the displacements have to agree -- and `verify_dir32_consistency` still proves the two sites name one object, because it subtracts each site's addend. Then address the buffer from its own base with non-negative indices and let the compiler pick the cursor; the assignment ORDER of the loads is the remaining lever (here `r`, `g`, `b` for a BGRA row). Landed `WorldHeightMap::readTiles` 0x00747100 (614 B) on 2026-09-22 after eight sessions had blocked on the literal. |
...
| A virtual call uses the wrong vtable temporary register at one site but a later call to the same proven slot already matches | Model the first site's proven slot with a typed vtable struct and a one-argument `__fastcall` function pointer: on x86 its receiver goes in ECX, matching this no-explicit-argument `__thiscall` slot. At 0x0023A380 (369 B), this selected the retail EAX vtable temporary at the first site; retaining the virtual-member call at the second preserved retail's EDX temporary, making the body exact. The slot, return width, and receiver ABI are supported by the call site and matched build. This is a C++ codegen-shape lever for a proven slot, not permission to guess a method name or lift bytes. |
...
| PMF constants `{pfn, -100, 0}` and a `[obj+0x68]` vbtable walk | `inputs/reference/shims/objectdlink/ObjectDlinkPmf.h`: Object's vbptr is inherited from a base at +0x68 (vbtable[0]==0), DLINK base at +4; pass the PMF as a value. |
...
| An 861-byte constructor matches except `mov [esp+0x4c],esp` before a by-value string call while retail stores at `[esp+0x58]` | Narrow the immediately preceding `Coord2D direction` local to a nested block ending after `setOrientation`; it is dead before the `findBridge(name)` by-value copy. VC7.1 reuses the later argument slot for that copy's EH saved-esp without changing any other instruction or the 30 relocation slots. At `Bridge::Bridge(BridgeInfo&, Dict*, AsciiString, int)` 0x001A98A0 this replaced the 861-byte emitted body with exact native C++. |
...
| A call to a C runtime function comes out as `call rel32` where retail has `call dword ptr [__imp__name]` | The translation unit is compiling against the static C runtime. Retail links the C runtime as a DLL, so every call reaches it through an import pointer. Give the source a `// cl:` line carrying `/MD`. Add `/D_STLP_USE_STATIC_LIB` to that line when the file is an STLport one, or `inputs/vendor/stlport/stl/_ios.h` fails with an explicit-specialization error. A `game/stlport/*.cpp` file with no `// cl:` line gets no `/MD` at all. That is what kept `__write_formatted_time` at 0x00846320 compiling 172 bytes long over its seven `sprintf` calls until 2026-09-16. |
...
| A vtable slot holds a body whose argument count and `ret N` belong to a different overload of the name you expected, and two neighbouring slots look swapped | MSVC 7.1 writes an overload set into the vtable in REVERSE declaration order. `_STL::ctype<char>` in `inputs/vendor/stlport/stl/_ctype.h` declares do_toupper(char), do_tolower(char), do_toupper over a range and do_tolower over a range, and the retail vtable at VA 0x0112F254 runs range-toupper, char-toupper, range-tolower, char-tolower. Match each slot on its argument count and its stack cleanup before you name it. This named 0x008407D0 and 0x008407C0 on 2026-09-16. |
...
| Callee "unpinned" in an old stash | Look it up again -- `targets/game/reverse/zh_sweep/requeue_resolved_callees.csv` lists 72 bodies whose blockers have since been pinned. |
...
| Retail loads a value (an argument's AsciiString data, a member) once before a loop and keeps it in a stack slot across a call in the loop body; ours reloads it after every call | The call is to a function whose body MSVC 7.1 can see, so interprocedural analysis proves it writes no memory. Give the callee its real body in the TU and mark it `__declspec(noinline)` so the call still binds to the pinned symbol. A declaration-only callee forces the reload; an inline body gets partially inlined instead. `/Oa` also caches the load but drops retail's other reloads, so it is the wrong lever. Landed the LightPointSystem level-index collector at 0x0039CB60 (314 B) on 2026-09-24 with the recursive `Overridable::friend_getFinalOverride` given its body. |
| Retail keeps an out-parameter local in a dead incoming-argument slot and ours gives it a frame slot, and `/FAsc` shows the argument slot unused | Pass the dead parameter itself as the out-parameter (`GadgetComboBoxGetSelectedPos(box, &itemData)` once `itemData` is no longer needed) instead of declaring a new local. Declaring the local at function scope, in a block, or with a split initialiser left it in the frame. Landed 0x00523460 (258 B) on 2026-09-24. |
...
| Records in a member array are read at the right offsets, but ours builds `lea esi,[eax+ebp+0x1b0]` and reads `[esi+0x40]` where retail builds `lea esi,[eax+ebx]` and reads `[esi+0x1f0]`; the register assignment then drifts for the rest of the body | Index the member array at every access (`m_trees[index].m_treeType`) instead of through `T &tree = m_trees[index]` or a record pointer: the reference folds the array's base offset into the `lea`. For a second table indexed by a value read from the first, copy that value into an `Int` local and write `m_treeTypes[type].m_data`. At `0x00734790`, `0x00736150` and `0x0071CF40` this brought the record and type addressing into line with retail; the `0x0071CF40` bank, written with a record pointer, had scored 0.25. |
...
Also check that the attempts log entry is not stale: `grep ,0xRVA, targets/game/reverse/functions.csv`
...
`game/GameEngine/Source/Common/Rva0035E710LinkWalk.cpp`. Inspect the callee's
...
`game/GameEngine/Source/Common/Rva003F0EC0Forward.cpp`.
...
Keep the BFME layout in the distinct `game/GameEngine/Include/Common/GlobalDataBFME.h`;
...
For a recovered translation unit, add `/Igame/GameEngine/Include` to its
...
The current `targets/game/reverse/functions.csv` contains 160 rows with this exact
...
`game/GameEngine/Include/GameClient/BfmeAptScreenBaseLayout.h`.
...
row in `game/GameEngine/Source/GameClient/Input/Rva005A6790.cpp`; the global
