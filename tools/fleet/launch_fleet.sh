...
# 2026-09-25: finish lane off by default. Its pool is empty under the attempt cap: 196 banked bodies at
# >=0.9 (80 KB), median 8 verdicts each, blocked on register allocation, which no further pass has moved.
# Its 2 seats go to mid, which now also serves named __emit lifts with proven extents (tools/lift_lane.py).
...
N=${1:-6}; B=${2:-0}; F=${3:-0}; M=${4:-14}; A=${5:-11}; R=${6:-2}; C=${7:-4}; K=${8:-1}
