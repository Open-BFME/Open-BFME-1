...
def repository(tmp_path):
    ...
    (root / "tools/re_log.py").write_text(
        ...
        "    with (Path(__file__).resolve().parents[1] / 'targets/game/reverse/re_attempts.log').open('a') as h:\n"
    ...
    (root / "targets/game/reverse/functions.csv").write_text(
        ...
        "foo,,0x1000,8,game/foo.cpp,matched,\n")
    (root / "targets/game/reverse/symbols.csv").write_text("name,address,notes\n")
    (root / "targets/game/reverse/re_attempts.log").write_text("")
    (root / "game/foo.cpp").write_text("int foo(){return 1;}\n")
    # a changed cited source is staged only once it byte-verifies on its own
    (root / "build.sh").write_text('echo "Functions: OK"\n', newline="\n")
    (root / ".gitignore").write_text("build/\nbuild.sh\ntargets/game/reverse/.*.lock\nhooks/\n")
    git(root, "add", "--", "tools", "targets/game/reverse", "game", ".gitignore")
...
def test_rebased_worktree_validator_blocks_publication(repository, tmp_path):
    ...
    (root / "game/foo.cpp").write_text("int foo(){return 2;}\n")
    result = harvest(root)
...
def test_network_does_not_hold_landing_lock_and_late_work_survives(repository):
    ...
    helper.write_text(
        ...
        "with (root / 'targets/game/reverse/.add_match.lock').open('a+') as h:\n"
        "    portable_lock.lock(h, exclusive=True)\n"
        "    (root / 'targets/game/reverse/re_attempts.log').write_text('late worker evidence\\n')\n"
    ...
    (root / "game/foo.cpp").write_text("int foo(){return 2;}\n")
    (root / "game/unrelated.cpp").write_text("unfinished unrelated source")
    result = harvest(root)
    assert result.returncode == 0, result.stdout + result.stderr
    assert "synchronization deferred" in result.stdout
    assert (root / "targets/game/reverse/re_attempts.log").read_text() == "late worker evidence\n"
    assert git(remote, "show", "master:game/foo.cpp") == "int foo(){return 2;}"
    assert "game/unrelated.cpp" not in git(remote, "ls-tree", "-r", "--name-only", "master")
...
def test_uncommitted_shared_dependency_cannot_contaminate_verification(repository):
    root, remote = repository
    before = git(remote, "rev-parse", "master")
    (root / "game/local.h").write_text("// unfinished dependency")
    result = harvest(root)
    assert result.returncode != 0
...
def test_own_leftover_index_is_recovered_not_refused(repository):
    ...
    root, remote = repository
    (root / "targets/game/reverse/re_attempts.log").write_text("late evidence\n")
    git(root, "add", "targets/game/reverse/re_attempts.log")
    (root / "build/.harvest_staged").write_text("1234")
    result = harvest(root)
    assert result.returncode == 0, result.stdout + result.stderr
    assert "unstaging the leftovers" in result.stdout
    assert "late evidence" in git(remote, "show", "master:targets/game/reverse/re_attempts.log")
    assert not (root / "build/.harvest_staged").exists()


def test_source_outside_the_official_tree_is_quarantined_not_fatal(repository):
    root, remote = repository
    (root / "game/stlport").mkdir()
    (root / "game/stlport/bad.cpp").write_text("int bad(){return 2;}\n")
    (root / "game/GameEngine").mkdir()
    (root / "game/GameEngine/good.cpp").write_text("int good(){return 3;}\n")
    with (root / "targets/game/reverse/functions.csv").open("a") as ledger:
        ledger.write("bad,,0x2000,8,game/stlport/bad.cpp,matched,\n"
                     "good,,0x3000,8,game/GameEngine/good.cpp,matched,\n")
    result = harvest(root)
    assert result.returncode == 0, result.stdout + result.stderr
    pushed = git(remote, "show", "master:targets/game/reverse/functions.csv")
    assert "good.cpp" in pushed and "bad.cpp" not in pushed
    assert (root / "build/quarantine/game/stlport/bad.cpp").exists()
    assert "placement" in (root / "targets/game/reverse/re_attempts.log").read_text()
