...
FUNCTIONS = ("name,export_rva,target_rva,target_size,source,status,notes\r\n"
             "?j_0002efbe@@YAXXZ,,0x0002EFBE,5,Code/gen_small/gthunks_052.cpp,matched,gen-thunk\r\n")
...
def repo(tmp_path, monkeypatch):
    ...
    (tmp_path / "Code/gen_small").mkdir(parents=True)
    (tmp_path / "reverse").mkdir()
    (tmp_path / "Code/gen_small/gthunks_052.cpp").write_text(THUNK)
    (tmp_path / "reverse/functions.csv").write_text(FUNCTIONS, newline="")
    monkeypatch.setattr(b_pin_check, "ROOT", tmp_path)

    def commit(pins):
        (tmp_path / "reverse/symbols.csv").write_text("name,rva,notes\r\n" + pins, newline="")
        git("add", "-A")
        git("commit", "-qm", "x")
...
def test_a_source_no_row_compiles_is_ignored(repo, tmp_path):
    (tmp_path / "Code/gen_small/orphan.cpp").write_text("void b_00123456();\nvoid f() { b_00123456(); }\n")
    sha = repo(PIN)
    assert b_pin_check.problems(sha) == []
