...
SOURCE = "Code/GameEngine/Source/Common/BfmeConv1270.cpp"
EVIDENCE = "reverse/identity_farthest_edge_001a38b0.md"
HEADER = b"name,export_rva,target_rva,target_size,source,status,notes\r\n"
OLD_ROW = f"{OLD},,0x001A38B0,151,{SOURCE},matched,".encode()
OTHER = b"?other@@YAXXZ,,0x001A4000,10,Code/other.cpp,matched,\n"
...
def repo(tmp_path):
    ...
    (tmp_path / "reverse/functions.csv").write_bytes(HEADER + OLD_ROW + b"\r\r\n" + OTHER)
...
def test_rename_preserves_mixed_terminators_and_records_tombstone(repo, monkeypatch):
    ...
    changed = (repo / "reverse/functions.csv").read_bytes()
    ...
    deleted = list(csv.reader(io.StringIO(
        (repo / "reverse/deleted_rows.csv").read_text())))[-1]
...
def test_failed_byte_gate_restores_both_ledgers(repo, monkeypatch):
    functions = repo / "reverse/functions.csv"
    deleted = repo / "reverse/deleted_rows.csv"
...
def test_preserves_existing_note_and_refuses_claimed_new_name(repo, monkeypatch):
    ...
    changed = functions.read_bytes().replace(OTHER, (
        OLD + ",,0x001A4000,10,Code/other.cpp,matched,\n").encode())
