...
def candidate(index):
    return {
        ...
        "path": f"game/candidate{index}.cpp",
        "line": index,
...
def main():
    ...
    proc = subprocess.run(
        [sys.executable, str(Path(__file__).resolve().parents[1] /
         ...
         "game/GameEngine/Source/Common/FireWeaponUpdateDestructorThunk.cpp", "--json"],
...
def test_partial_survives_the_legacy_three_field_filter(tmp_path, monkeypatch):
    ...
    log = tmp_path / "targets/game/reverse" / "re_attempts.log"
...
def test_stash_is_offered_with_a_served_candidate(tmp_path, monkeypatch):
    ...
    stash = tmp_path / "targets/game/reverse" / "attempts" / "0x00401000.cpp"
    ...
    try:
        ...
        assert found == ("targets/game/reverse/attempts/0x00401000.cpp", 0.5), found
...
def test_add_match_clears_the_stash_only_on_success(tmp_path):
    ...
    stash = tmp_path / "targets/game/reverse" / "attempts" / "0x000c8220.cpp"
    stash.parent.mkdir(parents=True, exist_ok=True)
    ...
    other = tmp_path / "targets/game/reverse" / "attempts" / "0x00401000.cpp"
