...
// Retail uses the BFME _STL list layout here: the list stores a pointer to a
// sentinel node at +0, whose next/prev links are at +0/+4 and whose drawable
// payload is at +8.  This view keeps that ABI local while the older InGameUI
// view above remains owned by its already-matched callbacks.
...
// 0x004A5950 is the already-bound retail context-command body reached by the
// dispatcher branch at the start of 0x004A9620.  Its semantics are kept as an
// RVA-derived view because the old d_ row is not an identity proof for its
// ControlBar name.
...
#pragma comment(linker, "/alternatename:?call@BfmeRva004A5950ControlBarContextCommandView@@QAEXXZ=?j_0003672d@@YAXXZ")

// 0x004A4240 receives the command, window, object, output-percent pointer and
// force flag in that order.  The retail callee writes 1.0f before evaluating
// the command, and returns its eight-way availability value.  This is a typed
// local view of the existing ILT, not a raw member or union bridge.
...
#pragma comment(linker, "/alternatename:?call@BfmeRva004A4240CommandAvailabilityView@@QBEHPBVCommandButton@@PAVGameWindow@@PAVObject@@PAM_N@Z=?j_000414d4@@YAXXZ")
...
protected:
	...
	char m_slice_padE[0x36C - (0x1F0 + MAX_COMMANDS_PER_SET * 4)];
	Int m_buildUpClockColor;									// this+0x36C
...
// ControlBar::updateContextMultiSelect, retail 0x004A9620, 655 bytes.
//
// The BFME body keeps a dispatcher guard that sends a non-null current
// selection through the retail 0x004A5950 context-command body.  The normal
// path is the multi-selection pass from the reference ControlBar family,
// with BFME's eight availability values, four status clears, inverse-clock
// overlay, and twenty-slot common-command tail retained.
//
// ?updateContextMultiSelect@ControlBar@@QAEXXZ
void ControlBar::updateContextMultiSelect(void)
{
	...
	BfmeControlBarDrawableListNode *head = selectedDrawables->head;
	BfmeControlBarDrawableListNode *it = head->next;
	GameWindow **commandWindows = m_commandWindows;
	...
	while (it != head)
	{
		...
		if ((*(const Int *)((const char *)thingTemplate + 0xCC) & 0x00800000) != 0)
		{
			it = it->next;
			continue;
		}
		...
		for (i = 0; i < MAX_COMMANDS_PER_SET; ++i)
		{
			win = commandWindows[i];
			if (win == 0 || win->winIsHidden())
				continue;
	...
	{
		win = commandWindows[i];
		if (win == 0 || win->winIsHidden() || m_commonCommands[i] == 0)
			continue;

		win->winEnable(objectsThatCanDoCommand[i] > 0);
	}
}
