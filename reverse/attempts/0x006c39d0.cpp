...
// partial score=0.17 date=2026-09-24
...
#define BFME_RADAR_TEXTURE_WIDTH \
	(*reinterpret_cast<Int *>(reinterpret_cast<char *>(this) + 0x14a4))
#define BFME_RADAR_TEXTURE_HEIGHT \
	(*reinterpret_cast<Int *>(reinterpret_cast<char *>(this) + 0x14a8))
void W3DRadar::buildTerrainTexture( TerrainLogic *terrain )
{
	*reinterpret_cast<Bool *>(reinterpret_cast<char *>(this) + 0x14dc) = TRUE;
	AsciiString mapName = TheGameState->getPristineMapName();
	Int dot = mapName.getLength() - 1;
	while (dot > 0 && mapName.getCharAt(dot) != '.')
		--dot;
	if (dot > 0)
	{
		...
		{
			TextureClass *&mapArt = *reinterpret_cast<TextureClass **>(
				reinterpret_cast<char *>(this) + 0x147c);
			if (mapArt)
			{
				mapArt->Release_Ref();
				mapArt = 0;
			}
			Rva0044F4D0 candidate((int)mapName.str());
			TextureClass *texture = *reinterpret_cast<TextureClass **>(&candidate);
			if (texture)
			{
			...
			*reinterpret_cast<Bool *>(reinterpret_cast<char *>(this) + 0x14dd) = TRUE;
			reinterpret_cast<Gen006C1A20ImageState *>(this)->initialize();
			return;
		}
	}
	*reinterpret_cast<Bool *>(reinterpret_cast<char *>(this) + 0x14dd) = FALSE;
	W3DRadarResetLock();
	W3DRadarResetSurface surface = reinterpret_cast<W3DRadarResetTexture *>(
		reinterpret_cast<char *>(this) + 0x1478)->getSurfaceLevel();
	RGBColor waterColor;
	...
	Coord3D worldPoint;
	Bridge *bridge;
	for( y = 0; y < BFME_RADAR_TEXTURE_HEIGHT; y++ )
	{

		for( x = 0; x < BFME_RADAR_TEXTURE_WIDTH; x++ )
		{
			...
			if( workingBridge == FALSE && terrain->isUnderwater( worldPoint.x, worldPoint.y, &waterZ ) )
			{
				...
				for( j = y - waterSamplesAway; j <= y + waterSamplesAway; j++ )
				{
					...
					if( j >= 0 && j < BFME_RADAR_TEXTURE_HEIGHT )
					{

						for( i = x - waterSamplesAway; i <= x + waterSamplesAway; i++ )
						{

							if( i >= 0 && i < BFME_RADAR_TEXTURE_WIDTH )
							{
			...
			//
			reinterpret_cast<SurfaceClass *>(*reinterpret_cast<void **>(&surface))->DrawPixel( x, y, GameMakeColor( color.red * 255,
																							 color.green * 255,
																							 color.blue * 255,
																							 255 ) );
...
#undef BFME_RADAR_TEXTURE_WIDTH
#undef BFME_RADAR_TEXTURE_HEIGHT
