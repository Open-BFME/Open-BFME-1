...
// ??1DebugStackwalk@@AAE@XZ present-unmatched
...
int DebugStackwalk::StackWalk(Signature &sig, struct _CONTEXT *ctx, bool useFallback)
{
  ...
  else
  {
    ...
    here:
		  lea	eax,here
		  mov	storage.regEip,eax
		  mov	storage.regEbp,ebp
		  mov	storage.regEsp,esp
	  };
	  storage.frame.AddrPC.Offset = storage.regEip;
	  storage.frame.AddrFrame.Offset = storage.regEbp;
						 ...
						 &storage.frame,ctx ? &storage.context : NULL,NULL,gDbg._SymFunctionTableAccess,gDbg._SymGetModuleBase,NULL))
  {
	...
	if (sig.m_numAddr < 5 && useFallback && !Rva0088EAF0IsDebuggerPresent())
	{
		DWORD *stack = reinterpret_cast<DWORD *>(ctx ? ctx->Esp : storage.regEbp);
		HANDLE process = GetCurrentProcess();
		sig.m_numAddr = 1;
		while (sig.m_numAddr < Signature::MAX_ADDR)
		{
			DWORD address;
			if (!ReadProcessMemory(process, stack, &address, sizeof(address), NULL))
				break;
			++stack;
			...
			if (!ReadProcessMemory(process, reinterpret_cast<void *>(address - 12),
			...
			if (call && sig.m_numAddr < Signature::MAX_ADDR)
				sig.m_addr[sig.m_numAddr++] = address;
		}
	}
