// ?Rva009AFA60FilterHoriz@@YAXHPAEH@Z
...
//
// Four instances ship: a vertical-edge pair walking rows and a horizontal-edge
// pair walking columns, each in an eight-pixel and a twelve-pixel length.
// MSVC unrolls every one of them four deep.
...
void Rva009AFA60FilterHoriz(int index, unsigned char *ptr, int stride)
{
	...
	for (int i = 0; i < 8; ++i) {
		int v = ((ptr[-2 * stride] - ptr[stride]) + 3 * (ptr[0] - ptr[-stride]) + 4) >> 3;
		BFME_LIMIT_ADJUST(v, t)
