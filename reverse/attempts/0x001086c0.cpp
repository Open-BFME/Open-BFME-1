// ?addObject@Radar@@QAEXPAVObject@@@Z
...
extern unsigned rva00107A50MakeColor( void *colorData );
...
{
	typedef RadarPriorityType (Object::*Call)() const;
...
static inline Bool BfmeIsKindOf( const Object *object, KindOfType kind )
{
	...
	union { void (*raw)(void); Call member; } call;
	call.raw = j_0003251f;
	return (((const Thing *)object)->*call.member)(kind);
}

static inline Bool BfmeIsNeutralControlled( const Object *object )
{
	typedef Bool (Object::*Call)() const;
	union { void (*raw)(void); Call member; } call;
	call.raw = j_0001483a;
	return (object->*call.member)();
}
...
{
	typedef Color (Object::*Call)() const;
	union { void (*raw)(void); Call member; } call;
	call.raw = j_00009ca0;
	return (object->*call.member)();
}
...
{
	typedef Module *(Object::*Call)(NameKeyType) const;
	union { void (*raw)(void); Call member; } call;
	call.raw = j_0002ae23;
...
{
	typedef NameKeyType (NameKeyGenerator::*Call)(const char *);
	union { void (*raw)(void); Call member; } call;
	call.raw = j_0003add7;
...
{
	typedef Relationship (Player::*Call)(const Team *) const;
	union { void (*raw)(void); Call member; } call;
	call.raw = j_0003e77a;
	return (player->*call.member)(team);
}
	...
	if( BfmeIsKindOf( object, (KindOfType)7 ) )
	{
		...
		else
		{
			if( BfmeIsNeutralControlled( object ) )
				return;
			Player *localPlayer = Rva002EE330ThePlayers->getLocalPlayer();
			if( localPlayer == NULL )
				return;
...
allocate_radar_object:
	...
	newObj = (BfmeRadarObjectAdd *)operator new( 0x10 );
	if( newObj )
	{
		newObj->m_vtable = (void *)0x01088838;
		newObj->m_object = NULL;
		newObj->m_next = NULL;
		newObj->m_color = GameMakeColor( 255, 255, 255, 255 );
	}
	...
	if( BfmeIsKindOf( object, (KindOfType)0x57 ) )
	{
		...
		StealthUpdate *update = (StealthUpdate *)BfmeFindModule( object, key_StealthUpdate );
		if( update )
		{
			if( update->isDisguised() )
			{
				Player *clientPlayer = Rva002EE330ThePlayers->getLocalPlayer();
				Player *disguisedPlayer = ThePlayerList->getNthPlayer( update->getDisguisedPlayerIndex() );
				if( BfmeGetRelationship( player, clientPlayer->getDefaultTeam() ) != ALLIES &&
	...
	if( object->getContain() )
	{
		...
		player = object->getContain()->getApparentControllingPlayer(
			Rva002EE330ThePlayers->getLocalPlayer() );
		if( player )
			useIndicatorColor = false;
	}
	...
	{	
		newObj->m_color = BfmeMakeColor( (void *)player->getPlayerColor() );
	}

	// set a chunk of radar data in the object
	object->friend_setRadarData( (RadarObject *)newObj );
