// ?OnChat@LANAPI@@UAEXVUnicodeString@@I0W4ChatType@LANAPIInterface@@@Z
...
void LANAPI::OnChat( UnicodeString player, UnsignedInt ip, UnicodeString message, ChatType format )
{
	...
	switch (format)
	{
		...
		case LANAPIInterface::LANCHAT_EMOTE:
		{
			...
			unicodeChat.concat(L' ');
			unicodeChat.concat(message);
			BfmeNetAddressView *sender = reinterpret_cast<BfmeNetAddressView *>(ip);
			BfmeNetAddressView *local = reinterpret_cast<BfmeNetAddressView *>(
				reinterpret_cast<unsigned char *>(this) + 0x44);
