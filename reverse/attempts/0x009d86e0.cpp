...
// TU-local shim: the writer derives from the save-side transfer base. The
// base occupies the witnessed vptr/stream/flag/position prefix; the writer's
// two hash maps follow it at +0x18 and +0x2c.
...
void Rva009D8630BlockWriter::rva009D86E0(const char *name)
{
	...
	{
		const Rva009D86E0String key(name);
		m_names[key] = static_cast<int>(m_aux.size()) +
