// ??0BfmeAptScreenQuitMenu@@QAE@PAX@Z
...
class QuitMenuArgHolder
{
public:
	__forceinline QuitMenuArgHolder( QuitMenuBinding binding )
	{
		m_ptr = new Rva0050F920FunctorWrapper(
			*(FunctorBinding *)&binding );
		if( m_ptr )
			++*(unsigned int *)( (char *)m_ptr + 4 );
	}
	...
	*(const void ***)( (char *)this + 0x218 ) =
	...
	if( g_obj12F4B40 == 0 )
	{
...

		{
			...
			QuitMenuBinding binding = {
				(FunctorTarget *)this, 0,
				(FunctorMethod)&BfmeAptScreenQuitMenu::_bfme_hasFocus };
			registry->showAptScreenWithArg(
				name, (void *)0,
				QuitMenuArgHolder( binding ) );
		}
			...
			gameLogic->m_gameMode == 5 ) )
		{
			...
			UnicodeString pauseText = TheGameText->fetch( "GUI:Menu" );
			g_theWindowManager->bfme_setAptText( pause, pauseText );
		}
	}
}
