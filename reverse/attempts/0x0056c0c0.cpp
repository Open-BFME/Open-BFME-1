// ?_bfme_onInitGadget@BfmeAptScreenSaveLoad@@QAEXPBDPAXPAVGameWindow@@@Z
	...
	const char *name, void *, GameWindow *window)
{
	if (window != 0)
	{
		...
		{
			UnicodeString emptyString((const unsigned short *)0x01088AF4);
			m_fileNameTextEntry = window;
			GadgetTextEntrySetText(window, emptyString);
