// ?onBodyDamageStateChange@BridgeBehavior@@UAEXPBVDamageInfo@@W4BodyDamageType@@1@Z
...
public:
	...
	virtual void unused15(); virtual void unused16();
...
public:
	virtual void unused00(); virtual void unused01(); virtual void unused02();
	virtual void unused03(); virtual void unused04();
...
struct BridgeDamageStateView
{
	unsigned char m_unreconstructed00[0x18];
	ObjectCreationList *m_damageToOCL[4][3];
	FXList *m_damageToFX[4][3];
	AudioEventRTS m_damageToSound[4];
	ObjectCreationList *m_repairToOCL[4][3];
	FXList *m_repairToFX[4][3];
	AudioEventRTS m_repairToSound[4];
	bool m_fxResolved;
	unsigned char m_unreconstructed459[0x460 - 0x459];
...
// ?onBodyDamageStateChange@BridgeBehavior@@UAEXPBVDamageInfo@@W4BodyDamageType@@1@Z
void BridgeBehavior::onBodyDamageStateChange(const DamageInfo *,
	BodyDamageType oldState, BodyDamageType newState)
{
	BridgeDamageStateView *state = (BridgeDamageStateView *)this;
	BridgeBehavior *primary = (BridgeBehavior *)((unsigned char *)this - 0x24);
	if (newState != BODY_RUBBLE)
		state->m_deathFrame = 0;

	if (!state->m_fxResolved)
		primary->resolveFX();
	if (!state->m_fxResolved)
		return;

	Object *object = *(Object **)((unsigned char *)this - 0x1C);
	Bridge *bridge = TheTerrainLogic->findBridgeAt(object->getPosition());
	if (bridge == 0)
		return;
	...
	if (gotRepaired)
	{
		TheAudio->addAudioEvent(&state->m_repairToSound[newState]);
		for (int i = 0; i < 3; ++i)
			primary->doAreaEffects(bridgeTemplate, bridge,
				state->m_repairToOCL[newState][i],
				state->m_repairToFX[newState][i]);
	}
	else
	{
		TheAudio->addAudioEvent(&state->m_damageToSound[newState]);
		for (int i = 0; i < 3; ++i)
			primary->doAreaEffects(bridgeTemplate, bridge,
				state->m_damageToOCL[newState][i],
				state->m_damageToFX[newState][i]);
	}
