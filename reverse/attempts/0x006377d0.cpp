...
void SetUpGameSpy(const char *motdBuffer, const char *configBuffer)
{
	...
	dir.format(AsciiString("%sGeneralsOnline"), reinterpret_cast<const Rva006C9270GlobalData *>(TheWritableGlobalData)->getPath_UserData().str());
	CreateDirectory(dir.str(), NULL);
	dir.format(AsciiString("%sGeneralsOnline\\Ladders"), reinterpret_cast<const Rva006C9270GlobalData *>(TheWritableGlobalData)->getPath_UserData().str());
	CreateDirectory(dir.str(), NULL);
