...
// The other declarations reproduce only the Shell surface this TU uses.
class Shell
{
public:
	...
	Bool isShellActive()
	{
		return *(const Bool *)((const char *)this + 0x58);
	}
};
...
extern int __cdecl bfmeReady(int mode);

class Rva00110170SaveGameCallTarget
{
public:
	SaveCode saveGame(AsciiString filename, UnicodeString desc,
		SaveFileType saveType, SnapshotType which, Bool showMessage);
};
typedef SaveCode (Rva00110170SaveGameCallTarget::*Rva00110170SaveGameCall)(
	AsciiString, UnicodeString, SaveFileType, SnapshotType, Bool);
			...
			switch( key )
			{
				...
				case KEY_ESC:
				{
					...
					if( BitTest( state, KEY_STATE_UP ) )
					{
						...
						// BFME predates the Kris 1.01 patch that hid the delete confirmation here.
...
{
	void (*asFunction)();
	Rva00110170SaveGameCall asMember;
} saveGameCast;
saveGameCast.asFunction = j_00020cd4;
(reinterpret_cast<Rva00110170SaveGameCallTarget *>(TheGameState)->*
	saveGameCast.asMember)(
filename, selectedGameInfo->saveGameInfo.description, fileType, SNAPSHOT_SAVELOAD, TRUE);
