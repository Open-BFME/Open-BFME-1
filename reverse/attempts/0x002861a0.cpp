// ?createVisionObject@BattlePlanUpdate@@IAEXXZ
...
void BattlePlanUpdate::createVisionObject()
{
	...
	const BattlePlanCreateVisionDataRetailView *retailData = reinterpret_cast<const BattlePlanCreateVisionDataRetailView *>(
		*reinterpret_cast<const BattlePlanUpdateModuleData *const *>( reinterpret_cast<const char *>( this ) + 0x04 ) );

	if (retail->m_visionObjectID != INVALID_ID) // don't want two.
		return;

	Object *obj = *reinterpret_cast<Object *const *>( reinterpret_cast<const char *>( this ) + 0x08 );
	BfmeThingFactory *factory = reinterpret_cast<BfmeThingFactory *>( TheThingFactory );

	// get template of object to create
	const ThingTemplate *tt = factory->findTemplate( retailData->m_visionObjectName );
