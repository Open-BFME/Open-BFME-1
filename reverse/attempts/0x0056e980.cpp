...
// SaveLoad.apt uses the holder whose real constructor lives at retail
// 0x0056D9B0.  The constructor is declared here so the call remains tied to
// that existing body; the holder itself is passed by value by the registry.
class Rva0056D9B0FunctorHolder
{
public:
	Rva0056D9B0FunctorHolder( FunctorBinding binding );
...
// The SaveLoad registry call has the same retail entry point as the existing
// AptGameWindow overload, but its holder type is distinct.  A separate call
// view keeps the already-matched DisconnectScreen calls on their original
// overload and gives the SaveLoad call its own address-derived ledger name.
class BfmeSaveLoadAptRegistry
{
public:
	void _bfme_showAptScreen( const AsciiString &name,
...
typedef int (__cdecl *BfmeSaveLoadObfHook)( void *, void *, __int64 );

struct BfmeSaveLoadObfSlot
{
	BfmeSaveLoadObfHook m_hook;
	BfmeSaveLoadObfHook m_alternate;
	char m_unmodelled[ 0x80 ];
	void *m_a;
	void *m_b;
};

extern BfmeSaveLoadObfSlot g_Slot012BF440;
class BfmeSaveLoadObfState
{
public:
...
// is pinned at 0x0003C65A.  The address-derived name reflects the body the
// caller reaches through that ILT; this declaration keeps the call indirect.
int bfmeMake_0056E900( int a, int b );
int bfmeSaveLoadFallback( int a, int b );
short bfmeSaveLoadNameIndex( int a, int b );
int bfmeSaveLoadObfuscatedStep( int a, unsigned int seed );
	...
	: _bfme_AptGameWindow( context )
{
	...
	*(const void ***)( (char *)this + 0x218 ) =
		...
		if( g_bfmeReadyAG == 0 )
	{
		...
		int obfMarker = 0xB83453E7;
		for( ;; )
		{
			BfmeSaveLoadObfHook hook = g_Slot012BF440.m_hook;
			if( hook )
				goto saveLoadObfHot;
			if( g_Slot012BF440.m_alternate )
				goto saveLoadObfHot;
			goto saveLoadObfFallback;

		saveLoadObfHot:
			{
				void *a = g_Slot012BF440.m_a;
				void *b = g_Slot012BF440.m_b;
				BfmeSaveLoadObfState state( &obfValue, &obfMarker );
				obfValue = hook( a, b, (__int64)(int)&state );
			}
			goto saveLoadObfCompare;

		saveLoadObfFallback:
			obfValue = bfmeSaveLoadFallback( obfValue, obfMarker );

		saveLoadObfCompare:
			if( obfValue == (int)0x993BA311 )
				break;
			{
				FunctorMethod callback =
					(FunctorMethod)&BfmeAptScreenSaveLoad::_bfme_provide;
				short nameIndex = bfmeSaveLoadNameIndex( obfValue, obfValue );
				AsciiString name( BfmeSaveLoadCallbackNames[ nameIndex ] );
				BfmeSaveLoadShownWithArgHolder holder(
					FunctorBinding( callback, (FunctorTarget *)this ) );
				( (BfmeSaveLoadWindowManager *)g_theWindowManager )
					->bindShownWithArg( name, (void *)(int)nameIndex,
						*(Rva0050F920FunctorHolder *)&holder );
				(void)bfmeSaveLoadObfuscatedStep( obfValue, 0xBA792210u );
			}
		}

		{
			...
			BfmeSaveLoadInitGadgetHolder holder(
				FunctorBinding( callback, (FunctorTarget *)this ) );
			_bfme_setAptScreenRef( initGadgets,
				*(Rva0050F840FunctorHolder *)&holder );
		}

		m_field260 = TheBfmeGameLogic->isGamePaused();
		TheBfmeGameLogic->setGamePaused( true, 2, true );
		m_field280 = *(int *)( (char *)g_theWindowManager + 0x1B8 ) == 0;
		if( m_field280 )
			g_theWindowManager->bfme_showBackground( 0 );
	}
}
