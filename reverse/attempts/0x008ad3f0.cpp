...
inline void *operator new(unsigned int bytes, void *where)
{
	return where;
}
...
{
public:
	BfmeLookupResult *bfmeFind1024(int key);
};

class __declspec(novtable) GenBase008AD3F0Base
{
public:
	__forceinline GenBase008AD3F0Base()
		: m_04(-1), m_08(0), m_0c(0), m_10(0), m_14(0)
	{
		*(volatile unsigned int *)this = 0x0113693c;
	}
	virtual void invoke();
	virtual void go();
	...
	unsigned char m_14;
	unsigned char m_padding15[3];
	int m_18;
	unsigned int m_1c;
	int m_20;
	};

class __declspec(novtable) GenBase008AD3F0 : public GenBase008AD3F0Base
{
public:
	GenBase008AD3F0();
...
GenBase008AD3F0::GenBase008AD3F0()
{
	...
	BfmeTab1024 *table =
		(BfmeTab1024 *)((char *)*(void **)0x013387d8 + 8);
	BfmeLookupResult *result = table->bfmeFind1024(0x013385f8);
	BfmeLookupResult *valueResult = result->getValue();
	Rva0089C860State *state = m_10;
	BfmeStateNode *newNode =
		(BfmeStateNode *)((unsigned int)valueResult->m_node & ~1);
	BfmeStateNode *oldNode = (BfmeStateNode *)
		((unsigned int)state->m_node & ~1);
	if (newNode)
		newNode->slot0();
	if (oldNode)
		oldNode->slot1();
	...
	if (newNode->slot5() == 1)
		newNode = (BfmeStateNode *)((char *)newNode + 1);
	state->m_node = newNode;
}
