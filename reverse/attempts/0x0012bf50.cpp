...
// its field parser at 0x002A4190.  The prior bank matched all but two
// pointer-copy register bytes.  This retry tested the reference clear() member
// call and typed vector layout; the retained count asm is the known MSVC 7.1
// codegen blocker, while the pointer copy remains typed C++ and differs only
// by ECX versus retail EDX.
	...
	FXBoneInfo *last)
{
	...
	while (count > 0)
	{
		...
		destination->particleSystemTemplate = source->particleSystemTemplate;
		int nextCount = count;
		++source;
		++destination;
		count = nextCount - 1;
