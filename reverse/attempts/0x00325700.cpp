...
// stlport
// Retail 0x00325700, 374 bytes.
//
// ScriptConditions::evaluateSkirmishPlayerHasDiscoveredPlayer, matched to the
// Zero Hour twin at
// reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/GameEngine/Source/GameLogic/ScriptEngine/ScriptConditions.cpp:2648.
// BFME resolves both Parameters to PlayerMaskType via
// ScriptEngine::unidentified_0034DB40 (proven at
// Code/GameEngine/Source/GameLogic/ScriptEngine/ScriptEngineGetPlayerMaskFromParameter.cpp)
// instead of ZH's playerFromParam(), then walks players with
// PlayerList::getEachPlayerFromMask (proven at
// Code/GameEngine/Source/Common/RTS/PlayerList.cpp) instead of a direct
...
// Code/GameEngine/Source/Common/RTS/Player_addScience_bfme.cpp, reused
...
// Non-virtual out-of-line call: matches
// Code/GameEngine/Source/Common/RTS/PlayerList.cpp (retail 0x000DF4A0).
class BfmePlayerListShim
{
public:
	Player *getEachPlayerFromMask(PlayerMaskType &maskToAdjust);
};
extern BfmePlayerListShim *ThePlayers;
...
// Code/GameEngine/Source/Common/RTS/Player_addScience_bfme.cpp.
...
// reference/shims/sweep/GameLogic/Object.h, pulled in transitively.
	...
	Parameter *pSkirmishPlayerParm, Parameter *pDiscoveredByParm)
{
	...
	if (discoveredByMask == 0)
		return false;

	PlayerMaskType skirmishMask = TheScriptEngine->unidentified_0034DB40(pSkirmishPlayerParm);
	if (skirmishMask == 0)
		return false;

	Player *discoveredByPlayer = ThePlayers->getEachPlayerFromMask(discoveredByMask);
	if (!discoveredByPlayer)
		return false;

	Int discoveredByIndex = ((BfmePlayerIndexField *)discoveredByPlayer)->m_playerIndex;

	while (skirmishMask)
	{
		Player *player = ThePlayers->getEachPlayerFromMask(skirmishMask);
		if (player)
		{
