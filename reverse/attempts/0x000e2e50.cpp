// ??0PlayerTemplate@@QAE@ABV0@@Z
// partial score=0.34 date=2026-09-15
...
{
public:
	PlayerTemplateSoundEvent(const PlayerTemplateSoundEvent &that) : m_data(that.m_data)
	{
		PlayerTemplateSoundObject *source = (PlayerTemplateSoundObject *)m_data;
		if (source != 0)
...
// This four-byte reference-counted string copy constructor is the callee used
// by retail for every scalar AsciiString member in PlayerTemplate's copy body.
...
public:
	...
	Money m_money;
	RGBColor m_preferredColor;
	GameSpyGroupRoom m_startingBuilding;
	GameSpyGroupRoom m_startingUnits[MAX_MP_STARTING_UNITS];
	ProductionChangeMap m_productionCostChanges;
	PlayerTemplateProductionTimeChangeMap m_productionTimeChanges;
	ProductionVeterancyMap m_productionVeterancyLevels;
	ScienceVec m_intrinsicSciences;
	ScienceVec m_intrinsicSciencesMP;
	GameSpyGroupRoom m_purchaseScienceCommandSet;
	GameSpyGroupRoom m_purchaseScienceCommandSetMP;
	GameSpyGroupRoom m_specialPowerShortcutCommandSet;
	GameSpyGroupRoom m_specialPowerShortcutWinName;
	Int m_specialPowerShortcutButtonCount;
	GameSpyGroupRoom m_loadScreenMusic;
	Bool m_observer;
	Bool m_playableSide;
	Int m_intrinsicSPP;
	Int m_maxLevelMP;
	Int m_maxLevelSP;
	GameSpyGroupRoom m_scoreScreenImage;
	GameSpyGroupRoom m_loadScreenImage;
	GameSpyGroupRoom m_headWaterMark;
	GameSpyGroupRoom m_flagWaterMark;
	GameSpyGroupRoom m_enabledImage;
	GameSpyGroupRoom m_sideIconImage;
	GameSpyGroupRoom m_beaconTemplate;
	std::vector<AsciiString> m_initialUpgrades;
	std::vector<AsciiString> m_buildableHeroesMP;
	PlayerTemplateSoundEvent m_lightPointsUpSound;
	PlayerTemplateSoundEvent m_objectiveAddedSound;
	PlayerTemplateSoundEvent m_objectiveCompletedSound;
	GameSpyGroupRoom m_defaultPlayerAIType;
	GameSpyGroupRoom m_spellBook;
	GameSpyGroupRoom m_spellBookMP;
	Bool m_evil;
	GameSpyGroupRoom m_spellStoreCurrentPowerLabel;
	GameSpyGroupRoom m_spellStoreMaximumPowerLabel;
};

class PlayerTemplateCopyShim : public PlayerTemplateAssignShim
{
public:
	__declspec(noinline) PlayerTemplateCopyShim(const PlayerTemplateCopyShim &that) :
		PlayerTemplateAssignShim(that)
	{
	}
};

PlayerTemplateCopyShim *forcePlayerTemplateCopyShim(void *storage, const PlayerTemplateCopyShim &that)
{
	return new (storage) PlayerTemplateCopyShim(that);
}
