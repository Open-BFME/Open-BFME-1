// ?dup_002b0250@@YAXXZ
// partial score=0.94 date=2026-09-22
...
class BfmeStructureToppleFXListView
{
public:
...
#pragma comment(linker, "/alternatename:?bfmeIsBlocked@BfmeStructureToppleFXListView@@QBE_NXZ=?j_00011f77@@YAXXZ")
#pragma comment(linker, "/alternatename:?doFXObj@BfmeStructureToppleFXListView@@QBEXPBVObject@@0@Z=?j_00022bba@@YAXXZ")
...
struct BfmeStructureToppleObjectConditionsView
{
	unsigned char m_padding00[0x110];
	UnsignedInt modelConditionFlags[10];
	void notifyModelConditionChanged(void);
};

#pragma comment(linker, "/alternatename:?notifyModelConditionChanged@BfmeStructureToppleObjectConditionsView@@QAEXXZ=?j_0002191d@@YAXXZ")
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
// ?dup_002b0250@@YAXXZ present-unmatched
UpdateSleepTime StructureToppleUpdate::update( void )
{
	...
	if (m_toppleState == TOPPLESTATE_TOPPLING) {
		...
		// It does not make sense to have a structural integrity less than zero.
		...
		if (m_accumulatedAngle >= PI/2) {
			...
			{
				Object *building = getObject();
				BfmeStructureToppleFXListView *fx =
					(BfmeStructureToppleFXListView *)d->toppleDoneFXList;
	...
	if (m_toppleState == TOPPLESTATE_WAITINGFORDONE)
	{
		if (m_toppleFrame <= TheGameLogic->getFrame())
		{
			...
			if (*((unsigned char *)conditionFlags) & 0x20) {
				...
				((BfmeStructureToppleObjectConditionsView *)building)->
					notifyModelConditionChanged();
			}
			UnsignedInt &secondConditionFlags = conditionFlags[1];
			if ((secondConditionFlags & 0x08000000) == 0) {
				secondConditionFlags |= 0x08000000;
				((BfmeStructureToppleObjectConditionsView *)building)->
					notifyModelConditionChanged();
			}
