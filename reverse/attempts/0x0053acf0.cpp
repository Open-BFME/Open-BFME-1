...
public:
	...
	unsigned char *cursor( void ) const
	{
		return *(unsigned char *volatile *)&m_data;
	}
...
// ---------------------------------------------------------------------------
// 0x0053ACF0 -- the same range read as peek, but it also steps the range past
// the byte it returned. The emptiness test is the same computed end()
// comparison, and the pointer is loaded through begin() again for both the
// dereference and the step, which is what keeps the three separate member
// reads retail makes after the test.

// @?take@Gen0053ACD0@@QAEHXZ 0x0053ACF0
int Gen0053ACD0::take( void )
{
	if( m_range->begin() != m_range->end() )
	{
		int value = *m_range->begin();

		m_range->m_data = m_range->cursor() + 1;
		m_range->m_size = m_range->m_size - 1;
