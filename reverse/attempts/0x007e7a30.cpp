...
static __forceinline void SetAxisAngle(Matrix4D &matrix, const Coord3D &axis, float angle)
{
    float c;
    float s;
    __asm {
...
Matrix4D &Matrix4D::Set(const Coord3D &v, float w)
{
    SetAxisAngle(*this, v, w);
    return *this;
}
...
// ??0Matrix4D@@QAE@ABVCoord3D@@M@Z
Matrix4D::Matrix4D(const Coord3D &v, float w)
{
    SetAxisAngle(*this, v, w);
}
