// ?populateUnderConstruction@ControlBar@@QAEXPAVObject@@@Z
...
protected:
	...
	void showRallyPoint(const Coord3D *point);
...
// The callback at 0x004AF6A0 is registered by ControlBar::switchToContext at
// 0x0049E901.  Its command literal and the cancel-button path identify the
// callback as the under-construction population step.  These small views
// retain the retail ILT call boundaries where the BFME names are not proven
// by a matched source body.
...
extern void j_0001ff91(void);
extern void j_000330dc(void);
extern void j_000179bd(void);
extern void j_0002262e(void);
extern void j_00023d4e(void);
extern void j_0003367c(void);
extern void j_0001f0d7(void);
extern void j_00034b58(void);

class Rva000DF7F0Players
{
private:
	char m_slice_pad[0x0C];
...
class Rva0049CFE0PortraitCall
{
public:
	void apply(Object *obj);
};

class Rva0058C040Call
{
public:
	void invoke(void);
};

class Rva005976B0Call
{
public:
	void update(void *obj);
};

class Rva004B19E0Call
{
public:
	void apply(void *windows);
};

// Retail dispatches getRallyPoint through vtable+0x20.  The eight leading
// slots are deliberately opaque: only this callback's ABI is established.
class ExitInterface
{
public:
	virtual void slot00(void) = 0;
...
static void callGloInvoke(Glo012F4B98Type *object)
{
	typedef void (Glo012F4B98Type::*Call)(void);
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_0003367c;
	(object->*target.member)();
}

static Bool callObjectIsLocallyControlled(Object *object)
{
	typedef Bool (Object::*Call)(void) const;
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_0001ff91;
	return (object->*target.member)();
}

static Bool callPlayersInactive(Rva000DF7F0Players *players)
{
	typedef int (Rva000DF7F0Players::*Call)(void) const;
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_000330dc;
	return (players->*target.member)() != 0;
}

static Bool callPlayerActive(Player *player)
{
	typedef Bool (Player::*Call)(void) const;
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_000179bd;
	return (player->*target.member)();
}

static void callPortrait(ControlBar *bar, Object *object)
{
	typedef void (Rva0049CFE0PortraitCall::*Call)(Object *);
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_0002262e;
	(((Rva0049CFE0PortraitCall *)bar)->*target.member)(object);
}

static ExitInterface *callObjectExitInterface(Object *object)
{
	typedef ExitInterface *(Object::*Call)(void) const;
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_00023d4e;
	return (object->*target.member)();
}

static void callGloUpdate(Glo012F4B98Type *object, Object *underConstruction)
{
	typedef void (Rva005976B0Call::*Call)(void *);
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_0001f0d7;
	(((Rva005976B0Call *)object)->*target.member)(underConstruction);
}

static void callExitImageUpdate(void *receiver, void *windows)
{
	typedef void (Rva004B19E0Call::*Call)(void *);
	union
	{
		void (*raw)(void);
		Call member;
	} target;
	target.raw = j_00034b58;
	(((Rva004B19E0Call *)receiver)->*target.member)(windows);
}

// ControlBar::populateUnderConstruction, retail 0x004AF6A0, 627 bytes.
// The BFME callback extends the reference implementation with the twenty
// command-window reset, local-player enable state, and the exit-image vector.
void ControlBar::populateUnderConstruction(Object *objectUnderConstruction)
{
	if (objectUnderConstruction == 0)
	{
		if (Glo012F4B98 != 0)
			callGloInvoke(Glo012F4B98);
		return;
	}
	...
	for (Int i = 0; i < MAX_COMMANDS_PER_SET; ++i)
	{
		...
		{
			window->winHide(true);
			setControlCommand(window, 0);
		}
	}

	Bool overlayMode = *(const Int *)((const char *)TheControlBar + 0x2EC) == 1;
	std::vector<GameWindow *> exitImageWindows;
	Bool locallyControlled = callObjectIsLocallyControlled(objectUnderConstruction);
	Rva000DF7F0Players *players = (Rva000DF7F0Players *)Rva002EE330ThePlayers;
	Bool shouldOverlay = false;
	if (!callPlayersInactive(players))
		shouldOverlay = locallyControlled;

	const CommandButton *commandButton = findCommandButton("Command_CancelConstruction");
	GameWindow *window = m_commandWindows[0];
	if (commandButton != 0 && window != 0)
	{
		window->winHide(false);
		if (overlayMode && shouldOverlay)
		{
			window->winSetStatus(0x04000000);
			exitImageWindows.push_back(window);
		}
		else
		{
			window->winClearStatus(0x04000000);
		}

		setControlCommand(window, commandButton);
		if (locallyControlled)
		{
			window->winEnable(true);
			window->winClearStatus(0x40000000);
		}
		else if (!callPlayerActive(players->m_localPlayer))
		{
			window->winEnable(false);
			window->winSetStatus(0x40000000);
		}
	}

	updateConstructionTextDisplay(objectUnderConstruction);
	callPortrait(this, objectUnderConstruction);
	ExitInterface *exit = callObjectExitInterface(objectUnderConstruction);
	if (exit != 0)
		showRallyPoint(exit->getRallyPoint());

	if (Glo012F4B98 != 0)
		callGloUpdate(Glo012F4B98, objectUnderConstruction);

	callExitImageUpdate(*(void **)((char *)this + 0x2F0), &exitImageWindows);
}
