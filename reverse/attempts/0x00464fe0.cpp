// ?_bfme_showAptScreenWithArg@_bfme_AptGameWindow@@QAEXABVAsciiString@@PAXVRva0050F920FunctorHolder@@@Z
...
class Rva0050F920FunctorWrapper;
...
public:
	...
	void unidentified_0046DBC0( const AsciiString &name, void *argument,
		Rva0050F920FunctorWrapper *callback );
...
public:
	...
	Rva0050F920FunctorWrapper *pointer() const { return m_ptr; }
...
private:
	char m_unmodelledPrefix[ 0xC ];
	_STL::vector<AsciiString> m_withArgumentNames;
	char m_unmodelledTail[ 0x238 ];
};

void _bfme_AptGameWindow::_bfme_showAptScreenWithArg(
	const AsciiString &name, void *argument, Rva0050F920FunctorHolder callback )
{
	Rva0050F920FunctorWrapper *callbackPointer = callback.pointer();

	if( g_theWindowManager )
		m_withArgumentNames.push_back( name );

	if( callbackPointer )
		++callbackPointer->m_refCount;
	g_theWindowManager->unidentified_0046DBC0( name, argument, callbackPointer );
}
