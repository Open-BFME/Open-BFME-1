...
The old `reverse/symbols.csv` candidate `?OnHasMap@LANAPI@@UAEXI_N@Z` copied Zero Hour's scalar-IP signature. It identifies the right vtable slot and body, but the first argument's BFME type is wrong.
...
The body remains a generated placeholder. A current native reconstruction is banked at `reverse/attempts/0x00688cd0.cpp`; it compiles to 678 bytes against 709 retail bytes and does not establish a byte match. The pin correction only removes the misleading scalar signature and preserves the witnessed address-pair identity.
