...
struct BfmeShadowBufferEntry
{
	...
	unsigned int copy(void *a, unsigned short b, void *c,
		unsigned short *d);
};
...
struct BfmeShadowCall0
{
	void Call();
};

struct BfmeShadowCall1
{
...
struct BfmeShadowCall4
{
	unsigned int Call(void *a, unsigned short b, void *c, unsigned int d);
};

typedef void (BfmeShadowCall0::*BfmeShadowCall0Fn)();
typedef void (BfmeShadowCall1::*BfmeShadowCall1Fn)(void *);
...
void BfmeVolumetricShadowBufferOwner::buildBuffer()
{
	...
	while (entry != owner->m_entries)
	{
		...
		entry->update();
		entry->fill((unsigned char *)owner->m_scratch);
		j_000494fe();

		int indexCount = (int)entry->reserved10 * 6;
		if (indexCount <= 30000)
		{
			...
			{
				BfmeShadowCall0Fn lock;
				union { void *asVoid; BfmeShadowCall0Fn asMember; } lockCast;
			...
			unsigned int written = entry->copy(
				vertexWrite +
					(30000 - buffers->m_vertexCapacity),
