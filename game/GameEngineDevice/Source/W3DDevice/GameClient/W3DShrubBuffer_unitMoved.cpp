...
// W3DShrubBuffer::unitMoved, retail 0x00721B20, uses the 50 by 50 tree partition and the older
...
// ?unitMoved@W3DShrubBuffer@@QAEXPAVObject@@@Z
void W3DShrubBuffer::unitMoved(Object *unit)
{
				...
				if (tree->m_treeType < 0) {
					...
					if (radius*radius>delta.lengthSqr()) {
						...
						if (data != 0 && data->m_framesToMoveOutward > 0) {
							pushAsideTree(tree->m_drawableID,
