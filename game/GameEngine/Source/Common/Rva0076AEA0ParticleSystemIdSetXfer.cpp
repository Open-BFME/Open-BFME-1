// cl: /DNDEBUG /MD
// Retail 0x0076AEA0: byte-twin of 0x000EFB00 xferObjectIDSet (Rva000EFB00SetXfer.cpp).
// Same shape, two callees differ: the per-element xfer goes through
// BfmeParticleSystemXferHandle instead of bfmeCalcTGC, and the insert call
// goes through BfmeSub1028::bfmeFill1028 instead of BfmeObjectIDTree::insertUnique.
// Strings: "std::set" at 0x01085E5C and "Set must be empty on load" at 0x01085E3C
// (same constants as the twin -- shared string pool).
...
public:
	RbNode *m_header;
	unsigned int m_size;
	void bfmeFill1028(int *result, int *value);
};
...
extern void __cdecl BfmeParticleSystemXferHandle(Xfer &, void *);

Xfer *rva0076AEA0ParticleSystemIdSetXfer(Xfer *xfer, BfmeSub1028 *tree)
{
	Xfer *x = xfer;
	BfmeSub1028 *t = tree;
	...
	if (x->IsStoring())
	{
		...
		while (node != sent)
		{
			BfmeParticleSystemXferHandle(*x, &node->m_value);
	...
	else
	{
		...
		while (count)
		{
			--count;
			int value;
			BfmeParticleSystemXferHandle(*x, &value);
			int result;
			t->bfmeFill1028(&result, &value);
		}
	}
	return x;
}
