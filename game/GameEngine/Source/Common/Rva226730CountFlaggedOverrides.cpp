...
struct Rva226730Thing
{
	...
	{
		const Overridable *value = templateValue;
...
void Rva226730CountFlaggedOverrides::updateCount(void)
{
	...
	while (node != objects) {
		Rva226730Thing *thing = (Rva226730Thing *)
			((char *)node->object + 4);
		// Preserve retail's distinct base adjustment and template load.
		_ReadWriteBarrier();
		if (thing->isFlagged())
			++count;
