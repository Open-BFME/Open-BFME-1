// cl: /O2 /Ob1 /DNDEBUG /DWIN32 /D_WINDOWS /MD /EHsc /Iinputs/reference/shims/stringinline
...
float AudioManager::getAudioLengthMS(const AudioEventRTS *event) { AudioManagerMutex guard(m_mutex); if(!event->getAudioEventInfo()) { getInfoForAudioEvent(event); if(!event->getAudioEventInfo()) return 0.0f; } AudioEventRTS eventForLengthCalculation=*event; eventForLengthCalculation.bfmeGenerateFilename(); eventForLengthCalculation.generatePlayInfo(); return getFileLengthMS(eventForLengthCalculation.getAttackFilename()) + getFileLengthMS(eventForLengthCalculation.getFilename()) + getFileLengthMS(eventForLengthCalculation.getDecayFilename()); }
