...
// Open-BFME5: STLport reallocating insert for a 48-byte vector element, retail
// 0x0028DAB0, 346 bytes. The body carried only a machine byte-dump row and no name.
//
// The element type is not recoverable: the only named-shaped callee is the tail
// move's __uninitialized_copy at 0x0028D4E0, reached through the ILT at 0x000030DA,
...
//
// 48 bytes is what the bytes say: the element is trivially copied, so the two
// leading phases inline into 12-dword rep movsd runs behind the placement-new
