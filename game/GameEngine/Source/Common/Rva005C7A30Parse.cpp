...
// Address-derived model for the retail static cdecl INI field callback at 0x005C7A30.
...
public:
	...
	const char *m_name;
	const char *m_key;
	Rva005C7A30Factory *m_next;
};

class Rva005C7A30ModuleTemplate
{
public:
	virtual void destroy(unsigned int flags);
};

class Rva005C7A30Parse
{
public:
	static void parse(INI *ini, void *data, void *store, const void *userData);
};

// ?parse@Rva005C7A30Parse@@SAXPAVINI@@PAX1PBX@Z
void Rva005C7A30Parse::parse(INI *ini, void *data, void *, const void *)
{
	const char *token = ini->getNextToken(0);
	BFMERetailAsciiString name(token);
	Rva005C7A30Factory *factory = *reinterpret_cast<Rva005C7A30Factory **>(0x012F64EC);
	...
	void *old = *reinterpret_cast<void **>((char *)data + 0xA4);
	if (old)
