...
//     table shape in game/Libraries/Source/WWVegas/WWLib/RvaHashFindAsciiString.cpp)
...
class Rva00462DE0Table
{
public:
	void *findRaw(const AsciiString &key)
	{
		typedef void *(Rva00462DE0Table::*MemberThunk)(const AsciiString &);
		union {
			void (*function)(void);
			MemberThunk member;
		} thunk;
...
void *Rva00462DE0(const char *name)
{
	void *node;
	{
		...
		node = g_Rva00462DE0_table.findRaw(key);
	}
