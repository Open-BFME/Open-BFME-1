...
// with a virtual view.  This TU owns the actual LineEmissionVolume singleton
// ABI only.  Its four data words are the retail object at 0x012F6BCC: table,
...
typedef ModuleTag<5, LINE_EMISSION_VOLUME_MODULE_KEY,
	LINE_EMISSION_VOLUME_MODULE_NAME,
	LineEmissionVolumeModule, LineEmissionVolumeModuleTemplate,
	DefaultParticleModule<5>, DefaultParticleModuleTemplate<5> >
	LineEmissionVolumeTag;

// Retail globals: +4 reads [0x012F6D60], +8 reads [0x012F6D64], and the
...
extern ConcreteModuleClass<LineEmissionVolumeTag> *
	lineEmissionVolumeRegistryHead;
extern void *lineEmissionVolumeVtable[];

template <>
class ConcreteModuleClass<LineEmissionVolumeTag>
{
public:
	ConcreteModuleClass()
	{
		void *sourceAt8 = *lineEmissionVolumeSourceAt8;
		void *sourceAt4 = *lineEmissionVolumeSourceAt4;
		m_sourceAt8 = sourceAt8;
		m_sourceAt4 = sourceAt4;
		m_next = lineEmissionVolumeRegistryHead;
		lineEmissionVolumeRegistryHead = this;
		m_table = lineEmissionVolumeVtable;
	}

	~ConcreteModuleClass();
	static const ConcreteModuleClass<LineEmissionVolumeTag> &getInstance();
...
const ConcreteModuleClass<LineEmissionVolumeTag> &
ConcreteModuleClass<LineEmissionVolumeTag>::getInstance()
{
	static ConcreteModuleClass<LineEmissionVolumeTag> instance;
	return instance;
}
