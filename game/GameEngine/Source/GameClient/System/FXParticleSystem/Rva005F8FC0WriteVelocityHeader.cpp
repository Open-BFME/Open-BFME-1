// Retail 0x005F8FC0. The four velocity-template writeINI methods call this
// category-4 header writer before serializing their module-specific fields.
// The category-0 twin at Rva005F0380WriteDefaultHeader.cpp matches the same
// secondary module-class view and stream-writing sequence.
...
private:
	char padding_[0x50];
	C fill_;
	basic_streambuf<C, T> *streambuf_;
	basic_ios<C, T> *tie_;
};
...
enum ModuleCategory {
	// Retail passes selector 4 for this velocity-template header.
	...
	unsigned int *flags)
{
	...
	reinterpret_cast<StreamWriter *>(&stream)->append(
		GetKey(MODULE_CATEGORY_VELOCITY));
	reinterpret_cast<StreamWriter *>(&stream)->append(" = ");
