...
// with a virtual view.  This TU owns the actual QuadDraw singleton ABI only.
// Its four data words are the retail object at 0x012F69A4: table, the two
...
// Retail globals: +4 reads [0x012F6CA8], +8 reads [0x012F6CAC], and the
...
template <>
class ConcreteModuleClass<QuadDrawTag>
{
public:
	ConcreteModuleClass()
	{
		void *sourceAt8 = *quadDrawSourceAt8;
	...
	~ConcreteModuleClass();
	static const ConcreteModuleClass<QuadDrawTag> &getInstance();
...
const ConcreteModuleClass<QuadDrawTag> &
ConcreteModuleClass<QuadDrawTag>::getInstance()
{
	static ConcreteModuleClass<QuadDrawTag> instance;
	return instance;
}
