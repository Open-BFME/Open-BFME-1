...
// BfmeAptScreenOnlineCustomMatch start-position callback at retail
// 0x0053D7D0. The StartPos=%d literal and GameSlot +0x10 store identify the
// callback beside the matched online custom-match slot handlers.
...
class Rva0053D7D0OnlineCustomMatch
{
public:
...
// ?applySlotStartPos@Rva0053D7D0OnlineCustomMatch@@QAE_NPAVGameSlot@@H@Z
bool Rva0053D7D0OnlineCustomMatch::applySlotStartPos(
	GameSlot *slot, int startPos )
{
	if( !TheGameSpyInfo )
		return false;
	...
	{
		AsciiString options;
		options.format( AsciiString( "StartPos=%d" ), slot->getStartPos() );
