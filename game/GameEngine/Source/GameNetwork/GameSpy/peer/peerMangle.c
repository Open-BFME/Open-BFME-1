...
/* GameSpy Peer SDK -- peerMangle.c, the room/user name mangler.
   Reconstructed, not vendored, for the same reason as peerCallbacks.c next
   door: the Area 51 carrier that supplies the rest of this SDK ships no peer/
   module, so the 2004 text is not in hand and the bodies here are transcribed
   from the 2007 SDK (nitrocaster/GameSpy, src/GameSpy/Peer/peerMangle.c).
   Only the includes and the two length constants are local -- peerMain.h and
   peerMangle.h of this vintage are equally unavailable, and PI_ROOM_MAX_LEN /
   PI_USER_MAX_LEN are their values from the 2007 headers.

   Placement is anchored on retail's string references, not on shape:
   "#GPG!%d" is reached only from 0x00866BF0, "#GSP!%s!%c%s%c" only from
   0x00866C10, "X%sX|%d" only from 0x00866C80, and the three digit alphabets
   ("0123456789abcdef", "aFl4uOD9sfWq1vGp", "qJ1h4N9cP3lzD0Ka") only from
   0x00866A20 and 0x00866AE0, which are EncodeIP and DecodeIP.  The rest of the
   file follows in source order between those anchors. */
