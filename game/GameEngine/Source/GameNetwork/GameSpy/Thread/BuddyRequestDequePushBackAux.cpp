...
// Open-BFME5: _STL::deque<BuddyRequest>::_M_push_back_aux_v, retail
// 0x0063CD80, 137 bytes. The body carried only a machine byte-dump row;
// targets/game/reverse/reloc_names.csv holds the name with identity=real.
//
// BuddyRequest is 0x2B8 bytes, which is what the frame reserves and what the
// buffer allocation asks for. STLport's buffer_size is 512 / sizeof(T) or 1
...
protected:
	...
	void _M_reallocate_map(unsigned int nodesToAdd, bool addAtFront);	// ILT 0x0000C450
...
// ?_M_push_back_aux_v@?$deque@VBuddyRequest@@V?$allocator@VBuddyRequest@@@_STL@@@_STL@@IAEXABVBuddyRequest@@@Z
template <class T, class Alloc>
...
template void deque<BuddyRequest, allocator<BuddyRequest> >::_M_push_back_aux_v(const BuddyRequest &);
