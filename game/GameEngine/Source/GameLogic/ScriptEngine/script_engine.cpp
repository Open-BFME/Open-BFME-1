...
// Reconstructed from 0x003107F0 (57,072 B -- the largest function in the binary).
// Two details are load-bearing for the byte match:
//   * the `curTemplate` pointer. Retail addresses each block through a cached base
//     (lea edi,[esi+0x1c]; [edi+8]); indexing m_actionTemplates[i] directly instead
//     emits absolute offsets and diverges (61.66% vs 100%).
//   * every assignment is kept in retail's order, duplicates included -- see the
//     SET_BASE_CONSTRUCTION_SPEED block, where retail assigns m_uiStrings[1] twice.
...
void ScriptEngine::init()
{
	...
	// Retail really does assign [1] twice here (m_uiStrings[2] is never written even
	// though m_numUiStrings is 3) -- a copy-paste bug in the original. Kept verbatim.
	...
	curTemplate->m_uiStrings[1] = " flashes white for ";
	curTemplate->m_uiStrings[2] = " seconds.";
	curTemplate = &m_actionTemplates[227];
	...
	curTemplate = &m_actionTemplates[658];
	...
	curTemplate = &m_actionTemplates[659];
