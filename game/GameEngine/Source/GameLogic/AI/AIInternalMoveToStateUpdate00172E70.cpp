// Retail 0x00172E70: AIInternalMoveToState::update (1166 bytes).
// The original method name is owned by the 000488F6 ILT wrapper in
// AIInternalMoveToStateThunks.cpp. Its named thiscall destination is this shim.
// Independent AIMoveToState and AIStates callers establish the method identity.
// State+1C -> machine+10 -> Object; Object+204 -> AI. These BFME layouts
// are checked against every access in the full retail body, not the ZH ABI.
// The 003E9720 analyst pack proves the five-argument hub and removeGoal ABI.
// rva002774c0 keeps its address because the distance-query name is not proved.
// Locomotor+38 remains unnamed; retail compares that float directly.
// The tail snapshots AI+168 before getPreferredHeight and conditionally sets
// the byte arrival flag. BitFlags::testWord's explicit returns preserve the
// retail TEST operand order at +2F2. No assembly or new pins are used.
// ?update@AIInternalMoveToStateUpdateShim@@QAE?AW4StateReturnType@@XZ
// cl: /O2 /Ob1 /DNDEBUG /DWIN32 /D_WINDOWS /MD /EHsc /Igame/GameEngine/Include/Precompiled /Igame/Libraries/Source/WWVegas/WWMath
// stlport
//
...
#pragma comment(linker, "/alternatename:?rva002774c0@AIUpdateInterface@@QAEMXZ=?j_000333ac@@YAXXZ")
...
#pragma comment(linker, "/alternatename:?query@Rva001B7E90Receiver@@QAEMPAVObject@@@Z=?j_000230ab@@YAXXZ")
...
public:
	...
	virtual StateReturnType bfmeVSlot03() = 0;
...
class AIInternalMoveToStateUpdateShim : public AIInternalMoveToState
{
public:
...
public:
	...
	{
		if (m_bits._M_getword(bit) & m_bits._S_maskbit(bit)) return true;
		return false;
	}
...
#line 452 "game/GameEngine/Source/GameLogic/AI/AIInternalMoveToState_update_Bfme.cpp"
		}
		else
		{
	...
	Real onPathDistToGoal = ai->rva002774c0();
	Rva001B7E90Receiver *curLoco = ai->m_curLocomotor;
	if (curLoco != 0 &&
	...
	else
	{
		...
		if ((unsigned char)pathfinder->bfmeCellTypeTwo(
	...
	onPathDistToGoal = ai->rva002774c0();
	...
	if (curLoco != 0)
	{
		if (onPathDistToGoal < ((Locomotor *)curLoco)->m_field038)
			closeEnoughAtTail = true;
		Real extraDistance = ai->m_pathExtraDistance;
		Real preferredHeight = ((Locomotor *)curLoco)->getPreferredHeight();
		if (preferredHeight < extraDistance)
		{
