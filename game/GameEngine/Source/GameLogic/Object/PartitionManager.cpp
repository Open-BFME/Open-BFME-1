...
// (targets/game/reverse/exports.csv) puts Snapshot::~Snapshot at ??1Snapshot@@UAE@XZ 0x0005C520,
...
{
	const unsigned char *layout = reinterpret_cast<const unsigned char *>(this);
	const Bool useObject = *reinterpret_cast<const Bool *>(layout + 0x10);
	const Object *relationshipObject = *reinterpret_cast<const Object * const *>(layout + 0x08);
	Relationship relationship;
	if (!useObject)
		relationship = relationshipObject->getRelationship(objOther);
	else
		relationship = objOther->getRelationship(relationshipObject);
	const Int flags = *reinterpret_cast<const Int *>(layout + 0x0C);
	if ((flags & (1 << relationship)) != 0)
		return true;
	return false;
}
