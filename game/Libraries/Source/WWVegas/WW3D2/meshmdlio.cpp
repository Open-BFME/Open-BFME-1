 ...
 *   2/16/99    GTH : Moved into MeshModelClass                                                *
 *=============================================================================================*/
// BFME Boolean implementation: game/Libraries/Source/WWVegas/WW3D2/MeshModelVertexColors.cpp
 ...
 *   9/1/2000   gth : Added alternate material desc support                                    *
 *=============================================================================================*/
// byte-exact reconstruction: game/GameEngine/Source/Common/_read_texture_ids_MeshModelClass_IAE_AW4WW3DErrorType_AAVChuThunk.cpp
...
{
public:
	__forceinline operator BfmeTexVGS **()
	{
		return (BfmeTexVGS **)this;
	}
	...
	if (DefMatDesc->Get_Pass_Count() == 2) {
		...
		if (!DefMatDesc->ShaderArray[0] && !DefMatDesc->ShaderArray[1]) {
			...
			if (emissive_map_effect) {
				...
				// Retail calls TextureHolder::bind to assign pass 1's texture to pass 0.
				((TextureHolder *)&DefMatDesc->Texture[0][0])->bind(
					*(const BFMEWaterTrackTextureHandle *)&DefMatDesc->Texture[1][0]);
				if (DefMatDesc->TextureArray [1][0]) {
					if (!DefMatDesc->TextureArray [0][0]) {
						DefMatDesc->TextureArray [0][0] = NEW_REF(
							TexBufferClass,
							(*(int *)((char *)this + 0x24), "MeshModelClass::DefMatDesc::TextureArray"));
						for (int i = 0; i < *(int *)((char *)this + 0x24); i++) {
							((BfmeBufVGS *)DefMatDesc->TextureArray[0][0])->bfmeSetVGS(
								i, ((Gen_00929290 *)DefMatDesc->TextureArray[1][0])->bfmeGet(i));
						}
					}
				}
				...
				if (DefMatDesc->MaterialArray[1]) {
					uv_source = ((VertexMaterialClass *)((Gen_00928ee0 *)DefMatDesc->MaterialArray[1])->m(0))->Get_UV_Source(0);
				} else {
					DefMatDesc->Material[1]->Get_UV_Source(0);
				}
				if (DefMatDesc->MaterialArray[0]) {
					for (int i = 0; i < *(int *)((char *)this + 0x28); i++) {
						((VertexMaterialClass *)((Gen_00928ee0 *)DefMatDesc->MaterialArray[0])->m(i))->Set_UV_Source(0, uv_source);
					}
				} else {
