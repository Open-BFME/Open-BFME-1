...
// readable body of ?MulAdd@VectorProcessorClass@@: game/Libraries/Source/WWVegas/WW3D2/bw_render.cpp
// readable body of ?Transform@VectorProcessorClass@@: game/Libraries/Source/WWVegas/WW3D2/streakRender.cpp
...
#include "matrix4.h"
#include "wwdebug.h"
// cpudetect.h omitted: CPUDetectClass is referenced only inside the __ICL (Intel
// compiler) SSE asm paths, which MSVC excludes, so the header is dead weight here.
#include <memory.h>
...
// ?Prefetch@VectorProcessorClass@@SAXPAX@Z absent-from-retail
// Empty under MSVC (body is entirely inside the __ICL asm path), unreferenced in
// the shipped code, so the retail linker /OPT:REF dead-stripped it: no address to match.
