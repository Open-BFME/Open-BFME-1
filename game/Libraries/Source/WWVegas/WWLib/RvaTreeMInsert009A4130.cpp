...
// Retail 0x009A4130 is the four-argument STLport tree insertion body reached
// from the existing insert callers at 0x009A41E0 and 0x009A4350.  Its complete
// boundary is 176 bytes: the body allocates a 0x18-byte node and calls the
// existing 8-byte value constructor at 0x009A3520.  The current ledger's
// ThingTemplate _Identity spelling conflicts with that storage evidence (an
// identity node would be 0x14 bytes), so this TU deliberately keeps the key
// and mapped payload address-derived rather than asserting a false semantic
// owner.  The caller chain remains recorded in the ledger note.
...
struct Rva009A4130Target;
struct Rva009A4130Value
{
	char m_body[4];
};

typedef Rva009A4130Target *const Rva009A4130Key;
typedef _STL::pair<Rva009A4130Key, Rva009A4130Value> Rva009A4130Pair;
typedef _STL::_Rb_tree<Rva009A4130Key, Rva009A4130Pair,
	_STL::_Select1st<Rva009A4130Pair>, _STL::less<Rva009A4130Key>,
	_STL::allocator<Rva009A4130Pair> > Rva009A4130Tree;

// Retail 0x009A4130; caller chain is 0x009A41E0/0x009A4350.
Rva009A4130Tree::iterator BfmeRbTreeInsertAnchor009A4130(
	Rva009A4130Tree *tree, _STL::_Rb_tree_node_base *x,
	_STL::_Rb_tree_node_base *y, const Rva009A4130Pair &v,
