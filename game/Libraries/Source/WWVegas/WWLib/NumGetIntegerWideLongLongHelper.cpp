// cl: /O2 /MD
// STLport 4.5.3 wide signed __int64 __get_integer<__true_type> at
// retail 0x00834360 (537B).
// BFME's wide character ABI is unsigned short.  In particular, the VC7
// wide streambuf's int_type is unsigned short, so sbumpc must retain the
// QAEG return type used by the retail call sites.
...
namespace _STL
{
	...
	template <>
	struct numeric_limits<__int64>
	{
		static __int64 min()
		{
			return (__int64)0x8000000000000000i64;
		}

		static __int64 max()
		{
			return (__int64)0x7fffffffffffffffi64;
		}
	};
		...
		const string &grouping, const __true_type &)
	{
		...
		Integer over_base = (numeric_limits<Integer>::min)() /
			(Integer)base;
		...
		if (got > 0)
			value = overflow
				? (negative ? (numeric_limits<Integer>::min)()
	...
	template bool __get_integer<WideIterator, __int64>(
		WideIterator &, WideIterator &, int, __int64 &, int, bool, char,
