...
// Open-BFME5: _STL::hashtable<pair<const UnsignedInt, Int>>::_M_insert, retail
// 0x00389A20, 111 bytes. The body carried only a machine byte-dump row;
...
//
// The bucket index is computed inline as a plain divide: the hash is the
...
template <class T1, class T2>
void _Construct(T1 *p, const T2 &value);			// ILT 0x0002D074
...
private:
	void resize(size_type numElementsHint);			// ILT 0x00040CF0
...
typedef pair<const UnsignedInt, Int> BfmeUnsignedIntPair;

template BfmeUnsignedIntPair &hashtable<BfmeUnsignedIntPair, UnsignedInt, hash<UnsignedInt>,
	_Select1st<BfmeUnsignedIntPair>, equal_to<UnsignedInt>,
	allocator<BfmeUnsignedIntPair> >::_M_insert(const BfmeUnsignedIntPair &);
