...
// Open-BFME5: _STL::_Rb_tree<UnsignedShort, pair<const UnsignedShort, Int> >::_M_insert,
// retail 0x0037B600, 178 bytes. The body carried only a machine byte-dump row;
...
//
// The node is 24 bytes: colour, parent, left and right, then the eight-byte
// pair -- the two-byte key pads out to four in front of the Int.
//
// The key comparison is 16-bit, which is where the two bytes over its 176-byte
// siblings come from: the operand-size prefixes on the load and the compare. _Select1st makes the key the pair's first, which sits at the front of
...
void _Construct(T1 *p, const T2 &value);			// ILT 0x0002BCE2
...
struct BfmeRbTreeInsertAnchorHelper
{
	typedef _STL::pair<const UnsignedShort, Int> Value;
	typedef _STL::_Rb_tree<UnsignedShort, Value, _STL::_Select1st<Value>,
		_STL::less<UnsignedShort>, _STL::allocator<Value> > Tree;
