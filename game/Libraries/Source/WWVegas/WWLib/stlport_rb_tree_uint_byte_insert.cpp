...
// Open-BFME5: _STL::_Rb_tree<UnsignedInt, pair<const UnsignedInt, UnsignedByte> >::_M_insert,
// retail 0x001A9580, 176 bytes. The body carried only a machine byte-dump row;
...
// pair. _Select1st makes the key the pair's first, which sits at the front of
...
void _Construct(T1 *p, const T2 &value);			// ILT 0x0003A319
...
struct BfmeRbTreeInsertAnchorHelper
{
	typedef _STL::pair<const UnsignedInt, UnsignedByte> Value;
	typedef _STL::_Rb_tree<UnsignedInt, Value, _STL::_Select1st<Value>,
		_STL::less<UnsignedInt>, _STL::allocator<Value> > Tree;
