...
// Open-BFME5: _STL::_Rb_tree<AsciiString, pair<const AsciiString, HotKey> >::_M_insert,
// retail 0x005B2EF0, 183 bytes. The body carried only a machine byte-dump row;
...
//
// The node is 0x1C bytes: colour, parent, left and right, then the 12-byte
// pair, so HotKey itself is 8.
//
// The key comparison is a call rather than a cmp, because the key is an
// AsciiString: less<AsciiString> is specialised to a member compare on the
// left operand -- this on the value being inserted, the parent key pushed --
// tested against zero. That call and its test are the seven bytes this body
// has over its integer-keyed siblings. _Select1st makes the key the pair's first, which sits at the front of
...
// upstream layout: inputs/reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/GameEngine/Include/GameClient/HotKey.h
class HotKey
{
	char m_bfmeBody[8];
};
...
void _Construct(T1 *p, const T2 &value);			// ILT 0x0000474B
...
struct BfmeRbTreeInsertAnchorHelper
{
	typedef _STL::pair<const AsciiString, HotKey> Value;
	typedef _STL::_Rb_tree<AsciiString, Value, _STL::_Select1st<Value>,
		_STL::less<AsciiString>, _STL::allocator<Value> > Tree;
