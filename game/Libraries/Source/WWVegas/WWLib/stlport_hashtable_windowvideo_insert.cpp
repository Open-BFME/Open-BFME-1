...
// Open-BFME5: _STL::hashtable<pair<const GameWindow *const, WindowVideo *> >::_M_insert,
// retail 0x007871D0, 111 bytes. The body carried only a machine byte-dump row;
...
//
// The bucket index is computed inline as a plain divide: the manager's own
// hashConstGameWindowPtr is the identity on the pointer, so the key is read straight out of the pair and taken modulo the
...
// upstream layout: inputs/reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/GameEngine/Include/GameClient/WindowVideoManager.h
class WindowVideoManager
{
public:
	struct hashConstGameWindowPtr
	{
		unsigned int operator()(const GameWindow *const &window) const
		{
...
template <class T1, class T2>
void _Construct(T1 *p, const T2 &value);			// ILT 0x0001FCFD
...
private:
	void resize(size_type numElementsHint);			// ILT 0x0003ECAC
...
typedef pair<const GameWindow *const, WindowVideo *> BfmeWindowVideoPair;

template BfmeWindowVideoPair &hashtable<BfmeWindowVideoPair, const GameWindow *,
	WindowVideoManager::hashConstGameWindowPtr, _Select1st<BfmeWindowVideoPair>,
	equal_to<const GameWindow *>,
	allocator<BfmeWindowVideoPair> >::_M_insert(const BfmeWindowVideoPair &);
