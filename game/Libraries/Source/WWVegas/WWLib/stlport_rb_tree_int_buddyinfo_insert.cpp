...
// Open-BFME5: _STL::_Rb_tree<Int, pair<const Int, BuddyInfo> >::_M_insert,
// retail 0x004EBC90, 176 bytes. The body carried only a machine byte-dump row;
...
//
// The node is 0x30 bytes: colour, parent, left and right, then the 32-byte
// pair, so BuddyInfo itself is 28. _Select1st makes the key the pair's first, which sits at the front of
...
void _Construct(T1 *p, const T2 &value);			// ILT 0x00015F5A
...
struct BfmeRbTreeInsertAnchorHelper
{
	typedef _STL::pair<const Int, BuddyInfo> Value;
	typedef _STL::_Rb_tree<Int, Value, _STL::_Select1st<Value>,
		_STL::less<Int>, _STL::allocator<Value> > Tree;
