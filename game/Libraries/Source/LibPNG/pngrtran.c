...
/* libpng 1.0.5 (October 15, 1999) -- pristine upstream source, matching the
   ...
   /Os, not the repo default /O2: retail emits `or [esi+58h],18h` where /O2
   emits load/modify/store into a spare register.
   The PNG_NO_* set was recovered by scoring whole-function image hits per
   macro.  Each is pinned by a function whose retail size only reproduces
   with it: png_struct::mode sits at 0x58 (not 0x68) without the user
   transforms, and png_handle_sRGB is 226 bytes only without cHRM.
   NO_READ_bKGD implies NO_READ_BACKGROUND -- upstream 1.0.5 leaves
   pngrtran.c referring to png_do_background otherwise, and will not build. */
