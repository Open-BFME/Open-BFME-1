...
// RVA 00898F60, 591B. Member returns a constructed four-byte string (ret4).
// Opaque identity; layouts and hub ABI: docs/analysis/0x008985c0.md.
// Retail literals: _proto__ at VA0113604C, type at VA0112A89C, = and &.
...
Rva8CD130String Fields00898F60::underscoreFields00898F60()
{
    ...
    {
        Rva8CD130String key(it->m_key);
        if (key.text()[0] == '_')
        {
            if (strcmp(key.text() + 1, "_proto__") == 0) continue;
            if (strcmp(key.text() + 1, "type") != 0)
            {
                Rva8CD130Value *value = reinterpret_cast<Rva8CD130Value *>(it->m_taggedValue & ~1u);
                if (value->type() != 9 || value->isUndefined())
                {
                    value->getName(&text);
                    result.append(key);
