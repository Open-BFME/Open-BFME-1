// cl: /DNDEBUG /MD /EHsc
// RVA 0x008B3A40: wrap virtual slot 0x70 when value kind is 0x21.
class AptInteger
{
public:
...
struct Rva008B3A40Value
{
    void *m_vtable;
    unsigned int m_flags;
    char m_padding[0x18];
    Rva008B3A40Target *m_target;
};

AptInteger *aptNativeSlot70At008B3A40(Rva008B3A40Value *value, int argc)
{
    int result = 0;
    if ((value->m_flags & 0x3f) == 0x21 && value->m_target)
    {
        Rva008B3A40Target *target = value->m_target;
        result = target->slot70();
    }
