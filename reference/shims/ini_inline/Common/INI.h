...
public:
  ...
  static void parseMultiplayerStartingMoneyChoiceDefinition( INI* ini );
	...
	*/
	// Inline in BFME, not out-of-line as in ZH. Proven by the callers: every INI
	// field parser (parseReal, parseBool, parseLookupList, ...) carries this
	// function's own "Expected additional data after '%s'" message string in its
	// body, which only happens if the body is inlined into them. The out-of-line
	// copy still exists at 0x00850970 for callers that do not inline it.
