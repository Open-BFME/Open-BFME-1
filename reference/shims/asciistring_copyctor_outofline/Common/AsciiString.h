...
// -----------------------------------------------------
// SHIM: the copy constructor is declared and left UNDEFINED on purpose, so a
// by-value AsciiString temporary is built with a call to
// ??0AsciiString@@QAE@ABV0@@Z (retail 0x00887B60) instead of inlining the
// refcount bump. Retail constructs such a temporary with exactly that call --
// `lea eax,[src]; push eax; lea ecx,[dst]; call ctor' -- where the inline body
// is a load, a null test and an InterlockedIncrement, and never becomes an EH
// object at all. This is the same lever asciistring_outofline applies to
// releaseBuffer, one constructor further in.
//
// Everything else in this header is asciistring_outofline unmodified.

// -----------------------------------------------------
// SHIM: releaseBuffer is left undefined on purpose, so ~AsciiString emits a
// call to ?releaseBuffer@AsciiString@@IAEXXZ (retail 0x00887940) instead of
// inlining the refcount decrement. Retail destroys a by-value AsciiString
// parameter with exactly that call; the inline body is several instructions
// of refcount work that retail does not have at the call site.
// Everything else in this header is the reference copy, unmodified.
