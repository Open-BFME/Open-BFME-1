...
private:
	...
	// BFME string header is 8 bytes, not ZH's 4: it mirrors the WWLib
	// StringBase<T>::Header this engine actually links
	// (Code/Libraries/Source/WWVegas/WWLib/string_base.h) — a 4-byte ref count
	// followed by two shorts, so the character data starts at +8. Proven by
	// inlined AsciiString::str() bodies in retail: findImageByName @0x5D2CF0
	// and ModuleFactory::makeDecoratedNameKey @0x126260 both emit
	// `add eax, 8` / `lea ecx, [eax+8]` where ZH's 4-byte header emits +4.
	struct AsciiStringData
	{
		int				m_refCount;						// reference count
		unsigned short	m_len;								// length of data in use
