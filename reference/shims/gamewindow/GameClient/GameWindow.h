...
protected:
	...
	//   BFME inserts four pointer/word members that ZH does not have, shifting every
	//   member after each insertion point by +4. The offsets below are the retail
	//   [this+N] offsets each accessor body actually uses.
	//     +0x04  m_bfmeAnchor        (winSetPosition@0x4780d0 null-checks it and calls
	//                                  its vtable slot +8 before writing the region)
	...
	void *m_userData;										// @0x2c User defined data area
	WinInstanceData m_instData;					// @0x30 Class data, varies by window type (sizeof 0x1a8)
	void *m_inputData;								  // @0x1d8 Client data
	void *m_bfmeInputExtra;							// @0x1dc (BFME-only)

  // user defined callbacks
	GameWinInputFunc			m_input;					// @0x1e0 callback for input
	GameWinSystemFunc			m_system;					// @0x1e4 callback for system messages
	GameWinDrawFunc				m_draw;						// @0x1e8 callback for drawing
	GameWinTooltipFunc		m_tooltip;				// @0x1ec callback for tooltip execution
	void *m_bfmeCallbackExtra;					// @0x1f0 (BFME-only; setter body @0x478ed0)
	void *m_bfmeCallbackExtra2;					// @0x1f4 (BFME-only)

	GameWindow *m_next, *m_prev;	// @0x1f8,0x1fc List of sibling windows
