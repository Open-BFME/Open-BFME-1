...
//	virtual void OnSlotList( ReturnType ret, LANGameInfo *theGame ) = 0;															///< Slotlist for a game in setup
	...
	// BFME has six more virtuals ahead of fillInLANMessage: RequestLobbyLeave
	// @0x6850F0 calls it through vtable +0xC4 where this header puts it at
	// +0xAC. Names and signatures are unknown, so they are parked immediately
	// before the pinned slot and nothing below them moves.
	virtual void _bfme_lanapi_slot0( void ) = 0;
	virtual void _bfme_lanapi_slot1( void ) = 0;
	virtual void _bfme_lanapi_slot2( void ) = 0;
	virtual void _bfme_lanapi_slot3( void ) = 0;
	virtual void _bfme_lanapi_slot4( void ) = 0;
	virtual void _bfme_lanapi_slot5( void ) = 0;
...
struct LANMessage
{
	...
	// BFME's LANMessage is four bytes longer: RequestLobbyLeave reserves
	// 0x1DC of frame for one where this header needs 0x1D8. LANMessageType and
	// everything the function touches keep their offsets, so the extra bytes
	// are parked at the end.
	char _bfme_lanmessage_tail[4];
};
#pragma pack(pop)
