# Agent `port-wwmath-wwdebug`: port WWMath remainder + WWDebug
...
rules. This file is your **exclusive territory**. Your agent name is **`port-wwmath-wwdebug`**
(use it for `PROGRESS-port-wwmath-wwdebug.md` and `--author` checks).

## Your territory — own ONLY these

Reference directory: `reference/CnC_Generals_Zero_Hour/GeneralsMD/Code/Libraries/Source/WWVegas/WWMath  (and WWDebug/)/`
...
(Note: these span two dirs — the reference path for each is `.../WWVegas/<dir>/<file>`. Much of WWMath's remainder is the curve/spline family already proven ABSENT — expect many SKIPs; verify with locate.)
...
8. Commit via the README's **SAFE PUSH RITUAL**. Update `PROGRESS-port-wwmath-wwdebug.md`.
