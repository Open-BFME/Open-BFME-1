...
- Native `AIPlayer.cpp` and the vendored `AIPlayer.cpp` each define `clearTeamsInQueue()` as precisely those two queue-removal calls with one `deleteQueue` callback. The existing `targets/game/reverse/symbols.csv` pin independently names `?clearTeamsInQueue@AIPlayer@@IAEXXZ` at `0x00160E70`.
