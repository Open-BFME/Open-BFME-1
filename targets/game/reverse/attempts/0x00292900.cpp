...
UpdateSleepTime FireSpreadUpdate::update( void )
{
	...
	// BFME stores the aflame bit in the object status word at +0x90.  Keeping
	// this as the proven raw layout (also used by startFireSpreading below)
	// preserves the retail 0x400 mask and its register/flag shape.
	if( (*reinterpret_cast<const UnsignedInt *>(reinterpret_cast<const char *>(me) + 0x90) & 0x400) == 0 )
