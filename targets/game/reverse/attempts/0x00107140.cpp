...
class BfmeRadarClientFrame
{
public:
	virtual void radarSlot00( void );
	virtual void radarSlot01( void );
	virtual void radarSlot02( void );
	virtual void radarSlot03( void );
	virtual void radarSlot04( void );
	virtual void radarSlot05( void );
	virtual void radarSlot06( void );
	virtual void radarSlot07( void );
	virtual void radarSlot08( void );
	virtual void radarSlot09( void );
	virtual void radarSlot10( void );
	virtual void radarSlot11( void );
	virtual void radarSlot12( void );
	virtual void radarSlot13( void );
	virtual void radarSlot14( void );
	virtual void radarSlot15( void );
	virtual void radarSlot16( void );
	virtual void radarSlot17( void );
	virtual void radarSlot18( void );
	virtual void radarSlot19( void );
	virtual void radarSlot20( void );
	virtual void radarSlot21( void );
	virtual void radarSlot22( void );
	virtual void radarSlot23( void );
	virtual void radarSlot24( void );
	virtual void radarSlot25( void );
	virtual UnsignedInt getFrame( void );
};

#define TheGameClient (*(BfmeRadarClientFrame **)0x012F1464)
...
//-------------------------------------------------------------------------------------------------
//-------------------------------------------------------------------------------------------------
// ??0RadarObject@@QAE@XZ present-unmatched
...
/** CRC */
// ------------------------------------------------------------------------------------------------
// ?crc@RadarObject@@MAEXPAVXfer@@@Z present-unmatched
...
/** Load post process */
// ------------------------------------------------------------------------------------------------
// ?loadPostProcess@RadarObject@@MAEXXZ present-unmatched
...
struct BfmeRadarLookup
{
	unsigned char m_pad[0x28];
	BfmeRadarEventLookup m_event[64];
	Int m_nextFreeEvent;
	Int m_lastEvent;
	Int m_eventCursor;
	Int m_eventFrame;
...
//-------------------------------------------------------------------------------------------------
// ?update@Radar@@UAEXXZ present-unmatched
...
//-------------------------------------------------------------------------------------------------		
// ?radarToWorld2D@Radar@@QAE_NPBUICoord2D@@PAUCoord3D@@@Z present-unmatched
...
// ------------------------------------------------------------------------------------------------
// ?localPixelToRadar@Radar@@QAE_NPBUICoord2D@@PAU2@@Z present-unmatched
...
//-------------------------------------------------------------------------------------------------
// ?getLastEventLoc@Radar@@QAE_NPAUCoord3D@@@Z present-unmatched
static __forceinline void bfmeRadarGetLastEventLoc(BfmeRadarLookup *radar, Coord3D *destination)
{
	BfmeRadarClientFrame *client = TheGameClient;
	Int currentFrame = client->getFrame();
	Int cursor = radar->m_eventCursor;
	Int selected;
	if( cursor >= 0 )
	{
		Int lastEventFrame = radar->m_eventFrame;
		Int lastEvent = radar->m_lastEvent;
		Int eventAge = lastEventFrame - radar->m_event[ lastEvent ].m_createFrame;
		if( eventAge > 0 && currentFrame - lastEventFrame < 60 )
		{
			selected = cursor;
			while( TRUE )
			{
				...
				if( selected == cursor )
					break;
				Int type = radar->m_event[ selected ].m_type;
				if( type != RADAR_EVENT_INFILTRATION && type < RADAR_EVENT_NUM_EVENTS )
					break;
			}
			goto storeEvent;
		}
	...
	selected = radar->m_lastEvent;

storeEvent:
	radar->m_eventCursor = selected;
	Int eventIndex = selected;
	destination->x = radar->m_event[ eventIndex ].m_worldLoc.x;
	destination->y = radar->m_event[ eventIndex ].m_worldLoc.y;
	destination->z = radar->m_event[ eventIndex ].m_worldLoc.z;
	radar->m_eventFrame = TheGameClient->getFrame();
}

Bool Radar::getLastEventLoc( Coord3D *eventPos )
{
	BfmeRadarLookup *radar = (BfmeRadarLookup *)this;
	if( radar->m_lastEvent != -1 )
		goto haveLastEvent;
	return FALSE;

haveLastEvent:
	if( eventPos )
		bfmeRadarGetLastEventLoc( radar, eventPos );
...
/** CRC */
// ------------------------------------------------------------------------------------------------
// ?crc@Radar@@MAEXPAVXfer@@@Z present-unmatched
