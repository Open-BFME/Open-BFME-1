// ?PopulateReplayFileListbox@@YAXPAVGameWindow@@@Z
// partial score=0.27 date=2026-09-23
// Removing the unwitnessed trailing ReplayHeader dword makes the bank's
// 0x318-byte frame exact and moves the first non-relocation difference from
// +0x17 to +0x22; the remaining hand-patched CRC branch is not clean C++.
...
class Rva004E1450RecorderClass
{
public:
	struct ReplayHeader
	{
...
class Rva004E1450ReplayGameInfo : public Rva004E1450GameInfo
{
public:
	Rva004E1450ReplayGameInfo();
...
#pragma comment(linker, "/alternatename:??0Rva004E1450ReplayGameInfo@@QAE@XZ=?j_00016d47@@YAXXZ")
#pragma comment(linker, "/alternatename:?getMap@Rva004E1450ReplayGameInfo@@QBE?AVAsciiString@@XZ=?j_0002f28e@@YAXXZ")
...
void PopulateReplayFileListbox(GameWindow *listbox)
{
	...
	fileSystem->getFileListInDirectory(TheRecorder->getReplayDir(), asciisearch, replayFilenames, TRUE);
