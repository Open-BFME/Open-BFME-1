...
private:
	...
	SlotRange m_primary;
	unsigned char m_padC0[0x1c];
	SlotRange m_secondary;
};
...
extern Object *rva00372e40BuildObject(Object *, Object *, ThingTemplate *,
	void *);

Object *Gen_00371340::build(ThingTemplate *whatToBuild, Int slot,
	void *argument)
{
	SlotRange *range;
	if (whatToBuild == 0)
		return 0;
	range = &m_primary;
	if (whatToBuild->m_buildSlotState < 0)
		range = &m_secondary;
	...
	if (slot == -2)
	{
		...
		while (cursor.entry != range->end)
		{
			Object *result = rva00372e40BuildObject(
				(Object *)m_fieldA0, *cursor.entry, whatToBuild, argument);
			if (result != 0)
				return result;
			++cursor.entry;
		}
	...
	Int count = (Int)(range->end - range->begin);
	if (count > 0)
	{
		cursor.last = count - 1;
		if ((unsigned)slot > (unsigned)cursor.last)
			slot = cursor.last;
		if (slot < 0)
			slot = 0;
		return rva00372e40BuildObject(
			(Object *)m_fieldA0, range->begin[slot], whatToBuild, argument);
	}
	return 0;
}
