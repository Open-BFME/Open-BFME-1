...
# endif
  _STLP_TRY {
    // The retail initializer receives an already prepared locale subsystem.
    ...
    FILE *file_cin = stdin;
    FILE *file_cout = stdout;
    FILE *file_cerr = stderr;
    FILE *file_clog = stderr;

    // Initialize the four narrow stream objects.
    if (_S_was_synced) {
      ptr_cin->init(new stdio_istreambuf(file_cin));
      ptr_cout->init(new stdio_ostreambuf(file_cout));
      ptr_cerr->init(new stdio_ostreambuf(file_cerr));
      ptr_clog->init(new stdio_ostreambuf(file_clog));
    } else {
      ptr_cin->init(_Stl_create_filebuf(file_cin, ios_base::in));
      ptr_cin->init(_Stl_create_filebuf(file_cout, ios_base::out));
      ptr_cin->init(_Stl_create_filebuf(file_cerr, ios_base::out));
      ptr_cin->init(_Stl_create_filebuf(file_clog, ios_base::out)); 
    }
...
# ifndef _STLP_NO_WCHAR_T
    ...
    wfilebuf* win  = _Stl_create_wfilebuf(file_cin, ios_base::in);
    wfilebuf* wout = _Stl_create_wfilebuf(file_cout, ios_base::out);;
    wfilebuf* werr = _Stl_create_wfilebuf(file_cerr, ios_base::out);
    wfilebuf* wlog = _Stl_create_wfilebuf(file_clog, ios_base::out);
