...
// partial score=0.03 date=2026-09-09
...
extern volatile const Real g_bfmeLimitEK;
...
// BFME returns the radar texture's surface as a one-pointer value wrapper.  The
// matching accessor and destructor are kept TU-local here so renderObjectList
// uses the retail hidden-result and RAII ABI instead of the upstream raw pointer.
...
//-------------------------------------------------------------------------------------------------
__declspec(noinline) Bool legalRadarPoint( Int px, Int py )
{
...
void W3DRadar::renderObjectList( const RadarObject *listHead, TextureClass *texture, Bool calcHero )
{
	...
	// sanity
	W3DRadarResetTexture *resetTexture =
		reinterpret_cast<W3DRadarResetTexture *>( texture );
	if( listHead == NULL || resetTexture->texture == NULL )
		return;

	// get surface for texture to render into; BFME's accessor returns an owning
	// one-pointer value wrapper, which is layout-compatible with SurfaceClass.
	W3DRadarResetSurface surface =
		reinterpret_cast<W3DRadarResetTexture *>( texture )->getSurfaceLevel();
	SurfaceClass *surfaceClass = reinterpret_cast<SurfaceClass *>( &surface );
	...
	char *radarLayout = reinterpret_cast<char *>( this );
	Real radarWidth =
		(*reinterpret_cast<Real *>( radarLayout + 0x1448 ) -
		 *reinterpret_cast<Real *>( radarLayout + 0x143c )) / g_bfmeLimitEK;
	Real radarHeight =
		(*reinterpret_cast<Real *>( radarLayout + 0x144c ) -
		 *reinterpret_cast<Real *>( radarLayout + 0x1440 )) / g_bfmeLimitEK;
	...
	for( const RadarObject *rObj = listHead; rObj; rObj = rObj->friend_getNext() )
	{
		...
		Bool disguised = obj->isKindOf( BFME_KINDOF_DISGUISER );
		Bool skip = FALSE;
		...
		if( !disguised && obj->getRadarPriority() == RADAR_PRIORITY_LOCAL_UNIT_ONLY &&
		...
		radarPoint.x = pos->x / radarWidth;
		radarPoint.y = pos->y / radarHeight;
    ...
    // get the color we're going to draw in
		...
		if( disguised )
		{
			...
			UnsignedByte blue;
			UnsignedByte alpha;
			GameGetColorComponents(c, &red, &green, &blue, &alpha);
			red = (UnsignedByte)(red + ((255 - red) / 2));
			green = (UnsignedByte)(green + ((255 - green) / 2));
			blue = (UnsignedByte)(blue + ((255 - blue) / 2));
			c = GameMakeColor(red, green, blue, alpha);

		}  // end if
		else if (!rva006C42B0(obj, &c))
		{
			continue;
		}
		...
		if( legalRadarPoint( radarPoint.x, radarPoint.y ) )
			surfaceClass->DrawPixel( radarPoint.x, radarPoint.y, c );
