...
// The image-one callback constructs these seven BFME static lookup keys at
// entry.  Keep the string object at its proven eight-byte ABI locally; the
// existing retail constructor/destructor and ImageCollection lookup symbols
// provide the call routes without changing a shared header.
class AsciiString
{
public:
...
public:
	...
	const Image *enabledImage(Int index) const { return m_enabledDrawData[index].image; }
	const Image *disabledImage(Int index) const { return m_disabledDrawData[index].image; }
	const Image *hiliteImage(Int index) const { return m_hiliteDrawData[index].image; }
...
enum
{
	...
	WIN_STATUS_NOT_READY = 0x02000000,
	WIN_STATUS_ALWAYS_COLOR = 0x10000000,
...
// ?W3DGadgetPushButtonImageDrawOne@@YAXPAVGameWindow@@PAVWinInstanceData@@@Z
// The retail callback is a complete 0x00795140..0x00795BF8 body.  Its direct
// caller is W3DGadgetPushButtonImageDraw at 0x00795EE0; this is the local
// BFME reconstruction of the one-image path, including the overlay lookups
// observed in the retail prologue.
void W3DGadgetPushButtonImageDrawOne(GameWindow *window,
	WinInstanceData *instData)
{
	static const Image *cameoPush =
		TheMappedImageCollection->findImageByName("Cameo_push");
	static const Image *cameoHilited =
		TheMappedImageCollection->findImageByName("Cameo_hilited");
	static const Image *radialPush =
		TheMappedImageCollection->findImageByName("RadialPush");
	static const Image *radialOver =
		TheMappedImageCollection->findImageByName("RadialOver");
	static const Image *radialBorder =
		TheMappedImageCollection->findImageByName("RadialBorder");
	static const Image *radialClockOverlay2 =
		TheMappedImageCollection->findImageByName("RadialClockOverlay2");
	static const Image *radialClockOverlay1 =
		TheMappedImageCollection->findImageByName("RadialClockOverlay1");

	const Image *image = window->enabledImage(0);
	UnsignedInt status = window->winGetStatus();
	UnsignedInt state = instData->getState();

	// Explicit per-state art is suppressed by BFME's overlay-state mode.  The
	// additional high status bit is retained because retail also bypasses this
	// selection for its special image path.
	if (!(status & WIN_STATUS_USE_OVERLAY_STATES) &&
		!(status & 0x40000000))
	{
		if (!(status & WIN_STATUS_ENABLED))
		{
			image = (state & WIN_STATE_SELECTED) ?
				window->disabledImage(1) : window->disabledImage(0);
		}
		else if (state & WIN_STATE_HILITED)
		{
			image = (state & WIN_STATE_SELECTED) ?
				window->hiliteImage(1) : window->hiliteImage(0);
		}
		else if (state & WIN_STATE_SELECTED)
		{
			image = window->hiliteImage(1);
		}
	}
	...
	if (image)
	{
		...
		{
			if (!(status & WIN_STATUS_ENABLED) &&
				!(status & WIN_STATUS_NOT_READY))
			{
				if (status & WIN_STATUS_ALWAYS_COLOR)
					color = (Color)0xff909090;
				else
					mode = 4;
			}
		}
		drawImage(TheDisplay, image, (Real)start.x, (Real)start.y,
			(Real)(start.x + size.x), (Real)(start.y + size.y), color, mode);
	}
	...
	if (pData)
	{
		if (pData->overlayImage)
			drawImage(TheDisplay, pData->overlayImage,
				(Real)start.x, (Real)start.y,
				(Real)(start.x + size.x), (Real)(start.y + size.y), -1, 2);
		...
		if (pData->drawClock)
		{
			if (pData->drawClock == NORMAL_CLOCK)
				...
				TheDisplay->drawRemainingRectClock((Real)start.x,
					(Real)start.y, (Real)size.x, (Real)size.y,
					(Real)pData->percentClock, pData->colorClock);
			pData->drawClock = 0;
			window->winSetUserData(pData);
		}
		...
		if (pData->drawBorder && pData->colorBorder != WIN_COLOR_UNDEFINED)
			TheDisplay->drawOpenRect((Real)(start.x - 1),
				(Real)(start.y - 1), (Real)(size.x + 2),
				(Real)(size.y + 2), 1.0f, pData->colorBorder);
	...
	// Overlay-state art is layered after user data.  The two radial images are
	// kept in this same callback because retail initializes them at entry and
	// selects them only in the active overlay branches.
	if (status & WIN_STATUS_FLASHING)
		drawImage(TheDisplay, cameoPush, (Real)start.x, (Real)start.y,
			(Real)(start.x + size.x), (Real)(start.y + size.y), -1, 2);
	...
	if (status & WIN_STATUS_USE_OVERLAY_STATES)
	{
		if (status & WIN_STATUS_ENABLED)
		{
			if (state & WIN_STATE_HILITED)
			{
				drawImage(TheDisplay,
					(state & WIN_STATE_SELECTED) ? cameoPush : cameoHilited,
					(Real)start.x, (Real)start.y,
					(Real)(start.x + size.x), (Real)(start.y + size.y), -1, 2);
			}
			else if (state & WIN_STATE_SELECTED)
			{
				drawImage(TheDisplay, cameoPush, (Real)start.x,
					(Real)start.y, (Real)(start.x + size.x),
					(Real)(start.y + size.y), -1, 2);
			}
		}

		if ((state & WIN_STATE_SELECTED) && radialPush)
			drawImage(TheDisplay, radialPush, (Real)start.x, (Real)start.y,
				(Real)(start.x + size.x), (Real)(start.y + size.y), -1, 2);
		else if ((state & WIN_STATE_HILITED) && radialOver)
			drawImage(TheDisplay, radialOver, (Real)start.x, (Real)start.y,
				(Real)(start.x + size.x), (Real)(start.y + size.y), -1, 2);

		if (radialBorder && (status & WIN_STATUS_FLASHING))
			drawImage(TheDisplay, radialBorder, (Real)start.x,
				(Real)start.y, (Real)(start.x + size.x),
				(Real)(start.y + size.y), -1, 2);
		if (radialClockOverlay1 && !(status & WIN_STATUS_ENABLED))
			drawImage(TheDisplay, radialClockOverlay1, (Real)start.x,
				(Real)start.y, (Real)(start.x + size.x),
				(Real)(start.y + size.y), -1, 2);
		if (radialClockOverlay2 && (status & WIN_STATUS_NOT_READY))
			drawImage(TheDisplay, radialClockOverlay2, (Real)start.x,
				(Real)start.y, (Real)(start.x + size.x),
				(Real)(start.y + size.y), -1, 2);
	}
}
