// ?MapSelectMenuSystem@@YA?AW4WindowMsgHandledType@@PAVGameWindow@@III@Z
...
public:
	...
	explicit UnicodeString( const unsigned short *text );
...
{
public:
	__forceinline AsciiStringYH( const char *text ) : AsciiString( text ) {}
	~AsciiStringYH( void );
};

extern void __cdecl bfmeCommitYH( AsciiStringYH label );
																				  ...
																				  WindowMsgData mData1, WindowMsgData mData2 )
{
	...
	switch( msg ) 
	{
		...
		case GWM_CREATE:
		{
			...
			AsciiString buttonName("MapSelectMenu.wnd:ButtonBack");
			buttonBack = TheNameKeyGenerator->nameToKey(buttonName);
			...
			AsciiString radioButtonEasyAIName("MapSelectMenu.wnd:RadioButtonEasyAI");
			radioButtonEasyAI = TheNameKeyGenerator->nameToKey(radioButtonEasyAIName);
			...
			else if( controlID == buttonOK )
			{
				...
				if( selected != -1 )
				{
					...
					if (mapFname)
					{
						...
						NameKeyGenerator *nameKeyGenerator = TheNameKeyGenerator;
						NameKeyType headlessCountID = nameKeyGenerator->nameToKey(
							"MapSelectMenu.wnd:HeadlessCount" );
						GameWindow *headlessCount = windowManager->winGetWindowFromId(
							NULL, headlessCountID );
						GadgetComboBoxGetSelectedPos( headlessCount, (Int *)&g_bfmePtrAAV );
						bfmeCommitYH( AsciiStringYH( mapFname ) );
					}
				}  // end if
