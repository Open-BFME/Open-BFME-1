// ?W3DGadgetRadioButtonImageDraw@@YAXPAVGameWindow@@PAVWinInstanceData@@@Z
// partial score=0.425 date=2026-09-23
...
static __forceinline WinInstanceData *bfmeGetInstanceData( GameWindow *window )
{
	return reinterpret_cast<WinInstanceData *>( reinterpret_cast<char *>( window ) + 0x30 );
}
																	...
																	WinInstanceData *instData )
{
	...
	if( BitTest( instData->getState(), WIN_STATE_SELECTED ) )
	{
		...
		leftImage					= bfmeGetInstanceData( window )->m_hiliteDrawData[ 3 ].image;
		centerImage				= bfmeGetInstanceData( window )->m_hiliteDrawData[ 4 ].image;
		rightImage				= bfmeGetInstanceData( window )->m_hiliteDrawData[ 5 ].image;
		
	}
	else if( BitTest( window->winGetStatus(), WIN_STATUS_ENABLED ) == FALSE )
	{
		// disabled background
		leftImage					= bfmeGetInstanceData( window )->m_disabledDrawData[ 0 ].image;
		centerImage				= bfmeGetInstanceData( window )->m_disabledDrawData[ 1 ].image;
		rightImage				= bfmeGetInstanceData( window )->m_disabledDrawData[ 2 ].image;
		
	}  // end if
	else if( BitTest( instData->getState(), WIN_STATE_HILITED ) )
	{
		// hilited background 
		leftImage					= bfmeGetInstanceData( window )->m_hiliteDrawData[ 0 ].image;
		centerImage				= bfmeGetInstanceData( window )->m_hiliteDrawData[ 1 ].image;
		rightImage				= bfmeGetInstanceData( window )->m_hiliteDrawData[ 2 ].image;
		
	}  // end else if
	else
	{
		// enabled background 
		leftImage					= bfmeGetInstanceData( window )->m_enabledDrawData[ 0 ].image;
		centerImage				= bfmeGetInstanceData( window )->m_enabledDrawData[ 1 ].image;
		rightImage				= bfmeGetInstanceData( window )->m_enabledDrawData[ 2 ].image;
