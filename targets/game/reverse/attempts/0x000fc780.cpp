...
class BfmeGeometryInfo : public BfmeThingTemplateShadowSelector
{
public:
...
class BfmeTerrainLogicIterateView
{
public:
	virtual void slot00();
	virtual void slot01();
																			 ...
																			 void *funcUserData )
{
	...
	Real halfFootprintHeight = 0.0f;
	Real halfFootprintWidth = 0.0f;
	const BfmeGeometryInfo &geometry = *(const BfmeGeometryInfo *)(
		reinterpret_cast<const char *>( build ) + 0x60 );
	if( geometry.usePluralShadowName() )
	{
		halfFootprintHeight = geometry.boxMinorRadius();
		halfFootprintWidth = geometry.boxMajorRadius();
	}
	else
	{
			 ...
			 y += sampleResolution )
	{
				 ...
				 x += sampleResolution )
		{
			...
			// transform to world
			v.Set( x, y, reinterpret_cast<BfmeTerrainLogicIterateView *>( TheTerrainLogic )->getGroundHeight( x, y ) );
			...
			pos.x = v.X;
			pos.y = v.Y;
			pos.z = reinterpret_cast<BfmeTerrainLogicIterateView *>( TheTerrainLogic )->getGroundHeight( pos.x, pos.y );
